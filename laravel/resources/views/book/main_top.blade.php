<div class="page col-sm-offset-3 col-sm-9 main hidden-xs" id="load_content" style="display:none;">
  ...
  <div class="menu-wikidocs" style="height: 15px;"></div>
  <div class="clearfix page-depth">
    <div class="col-xs-9" style="color:#afb1af;">
      <small><a href="/book/{{$defurl}}"><i class="glyphicon glyphicon-folder-open"></i> {{$title}}</a> / </small>
<?php
  $bs_arr = [];
  for($i=1; $i<=$id; $i++){
    if($sub[$i][0]==1) {
...
?>
      <small><a href="/book/{{$defurl}}/{{max($bs_arr)}}">{{$sub[max($bs_arr)][1]}}</a></small>
      @if($sub[$id][0]==2) <small> / <a href="/book/{{$defurl}}/{{$id}}">{{$sub[$id][1]}}</a></small>@endif
    </div>
