  ...
  <div class="chapter">
    ...
    <section>
      <article>
        ...
        <p>다음 예제를 diva.py라은 이름의 파일을 생성하고 작성해 보자.</p>
        <h5>코드7-1 diva.py</h5>
        ...
        <p>diva.py 라는 파일은 현재 작업중인 곳과 같은 곳에 저장했다. 이 파일은 곧 파이썬 모듈이 된다. 그리고 이 모듈의 이름이 diva가 되는 것이다. 이제부터 위 예제의 diva 모듈에 정의되어 있는 Singer 클래스와 song() 메서드를 가져다 사용하는 법을 알아본다.</p>
      </article>
    </section>
    ...
    <section>
      <article>
        <p>7.1.1에서 살펴본 diva 모듈을 사용하려면 코드에 import diva라고 선언하면 된다. 이렇게 가져온 모듈 안의 함수 사용은 클래스에서 함수를 불러다 사용하는 것만큼이나 간단하다.</p>
        <h5>코드7-2 diva 모듈을 이용한 song() 메서드 실행</h5>
        ...
        <p>앞 파일에 정의 된 클래스 Singer를 사용해 보자.</p>
        <h5>코드7-3 Singer 클래스 사용</h5>
        ...
        <p>클래스를 사용하는 것처럼 '.'을 이용해 모듈 내부의 클래스, 함수, 변수에 접근할 수 있다.</p>
      </article>
    </section>
    ...
    <section>
      <article>
        <p>때로는 모듈 전체가 아니라 일부분만 필요한 상황도 있다. 이럴 땐 from &lt;모듈이름> import &lt;함수이름>을 이용하면 된다. 먼저 calcuater.py라고 만든 calculater 모듈을 살펴보자.</p>
        <h5>코드7-4 calculater 모듈</h5>
        ...
        <p>add(), mul(), div()라는 함수 3개가 있다. 여기서 add()함수만 가져다 사용하는 코드는 다음과 같다.</p>
        <h5>코드7-5 calculater 모듈의 add() 함수만 사용</h5>
        ...
        <p><strong>from calculater import *</strong> 처럼 해당 모듈의 모든 내용을 가져오면서 모듈 이름을 호출할 때 모듈명 없이 함수명만 호출할 수도 있다. 그러나 import calculater 로 호출한 후 모듈명.함수() 형식으로 불러오는 것이 코드의 내용을 이해하기 더 쉽다.</p>
      </article>
    </section>
    ...
    <section>
      <article>
        <p>사용하려는 모듈이나 모듈 내부의 함수 이름이 매우 긴 경우가 있다. 이럴 때 다른 이름으로 불러올 수 있다. 우선 [코드7-6]을 thisIsVeryLongNameModule 이라는 모듈로만들었다고 생각해보자.</p>
        ...
        <p>thisIsVeryLongNameModule 은 이름이 매우 길대 매번 이 모듈을 사용할 때마다 긴 이름을 입력하자면 번거롭다. 그럴 때는 다음과 같이 불러오면 된다.</p>
    ...
    <section>
      <article>
        <p>각 모듈은 셸에서 일반적인 python 명령을 입력해 사용할 수 있다.</p>
        ...
        <p>위 예제는 __name__을 검사해서 "__main__"과 같은지 확인한 후, 같으면 출력하게 한다. 정말 간단한 해결책이다. 앞 runable 모듈은 단독으로 사용하면 모듈 내부에 정의된 hello()를 실행한다.</p>
      </article>
    </section>
  </div>
  ...
  <div class="chapter">
    ...
    <section>
      <article>
        ...
        <blockquote>__init__.py 파일을 어떤 디렉터리 안에 넣으면 해당 디렉터리는 패키지가 된다.</blockquote>
...
0 directories, 9 files</code></pre>
        <p>이 9개의 파일 중 __init__.py 를 제외한 모든 파이썬 파일들은 모듈이다. 남은 __init__.py 가 해당 디렉터리를 패키지로 인식될 수 있게 해준다. 즉, 클래스 내부에서 __init__() 메서드가 해당 클래스를 초기화 시켜주는 것처럼 패키지 내부의 __init__.py 는 해당 디렉터리를 패키지로 인식하도록 초기화시켜 준다고 할 수있다.</p>
...
4 directories, 27 files</code></pre>
    ...
    <section>
      <article>
        ...
        <p>7.1에서 설명한 것처럼 from과 import를 이용해 원하는 모듈만을 가져오거나, 해당 모듈의 클래스나 함수들을 전부 가져올 수 있다.</p>

        <p>때로는 같은 패키지 않에서 다른 패키지나 모듈을 가져와야 할 상황이 있다. 그럴 때는 기존에 설명했던 '.'은 물론이고 '..'등을 사용할 수 있다. 예를 들어 overwatch.tank,.zarya 모듈에서 다른 모듈을 가져오려면 다음처럼 하면 된다.</p>
