  ...
  <div class="chapter">
    ...
    <section>
      <article>
        ...
        <p>transform_func()이라는 함수를 파라미터로 전달해서 해당 함수로 연산한 결과의 합을 계산한다. 여기서는 모든 파라미터와 반환 값을 대상으로 해당 함수를 실행했다. 이밖에도 이러한 방법을 활용할 수 있는 상황들이 있다.</p>
      </article>
    </section>
  </div>
  ...
  <div class="chapter">
    <section>
      <article>
        <p>[코드5-6] 처럼 매우 간단한 처리를 하기 위해 매번 통째로 함수를 선언한다는 것은 번거로운 일이다. 이런 경우 사용할 수 있는 것이 람다(lambda)이다.</p>
        ...
        <p>일단 람다를 사용하기 전에 '함수를 변수처럼 할당하거나 전달할 수 있다'라는 개념을 상기한다. 예를 들어 아래 add_1() 함수와 add_plus_one 변수는 같은 개념이다.</p>
        ...
        <p class="bg-info"><strong>NOTE_ 내장함수와 기본 라이브러리</strong><br>파이썬 역시 다른 프로그래밍 언어처럼 언어 안에 내장되어 사용할 수 있는 함수들이 있다. 이를 내장함수라고 한다. 예를 들어 print() 는 대표적인 내장함수이다. 파이썬 개발 문서의 '2. Built-in Functions(<a href="https://docs.python.org/3/library/functions.html" target="_blank">https://docs.python.org/3/library/functions.html</a>)' 기준으로 내장함수는 68개이다. 해당 개발 문서를 참고해서 내장함수에 무엇이 있는지 살펴보길 권한다.
        <br><br>또한 내장함수는 아니지만 파이썬 개발환경과 함께 설치되는 라이브러리에도 여러가지 함수가 있다. 'The Python Standard Library(<a href="https://docs.python.org/3.6/library/index.html#library-index" target="_blank">https://docs.python.org/3.6/library/index.html#library-index</a>)'에서 확인할 수 있다. 이 문서는 파이썬에 익숙해지면 자주 보게 될 것이다.</p>
      </article>
    </section>
  </div>