  ...
  <div class="chapter">
    <section>
      <article>
        ...
        <p>즉, 어떤 연구 과정에서 프로그래밍이 필요할 때 파이썬으로 쉽고 빠르게 아이디어를 실증해 볼 수 있다는 의미다. 이는 강력한 IDE를 제공하며, 컴파일하지 않고 변경사항을 즉시 확인할 수 있는 장점 때문이다. </p><p>또한 수 많은 라이브러리 덕분에 C나 자바 등으로 구현하기 번거로운 각종 반복 작업을 쉽게 구현해 자동화 할 수있고 운영체제를 가리지 않는 호환성은 어떤 환경에서 파이썬으로 구현한 프로그램을 실행해도 작업에 차질이 없게 한다. 2010년대에는 프로그래밍 교육용 언어로 파이썬을 도입하고 있다.</p>
      </article>
    </section>
  </div>
  ...
  <div class="chapter">
    ...
    <section>
      <article>
        ...
        <p>뷰티풀 수프는 HTML을 파싱하는데 사용하는 라이브러리로 구문 분석, 트리 탐색, 검색과 수정을 위한 관용구를 이용해 문서를 분석하고 필요한 것을 추출한다. 애플리케이션을 작성하는데 많은 코드가 필요하지 않고, 문서 인코딩을 자동으로 UTF-8로 변환하는 등의 기능이 있다. 또한 lxml이나 html5lib과 같은 인기 있는 파이썬 파서와 함께 사용할 수 있기도 하다.</p>
  ...
  <div class="chapter">
    ...
    <section>
      <article>
        <p>macOS는 파이썬 2.7만 기본 탑재하고 있다. 따라서 파이썬 3은 별도로 설치해야 한다.macOS에서 파이썬을 설치할 때는 설치파일을 다운로드해서 설치하는 방법과 '<code>Homebrew</code>'라는 패키지 관리자를 이용해 설치하는 방법이 있다.</p>
