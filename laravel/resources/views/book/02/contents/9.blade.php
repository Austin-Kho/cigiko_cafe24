  ...
  <div class="chapter">
    <section>
      <article>
        ...
        <p class="bg-warning"><strong>TIP</strong>: 파이썬의 파일 읽고 쓰기와 관련한 더 자세한 내용은 파이썬 개발 문서의 '7.2 Reading and Writing Files(<a href="https://docs.python.org/3.5/tutorial/inputoutput.html#reading-and-writing-files" target="_blank">https://docs.python.org/3.5/tutorial/inputoutput.html#reading-and-writing-files</a>)'를 참고하면 좋다.</p>
      </article>
    </section>
  </div>
  ...
  <div class="chapter">
    <section>
      <article>
        ...
        <p>파일을 여는데는 <a href="https://docs.python.org/3/library/functions.html#open" target="_blank">open()</a>함수면 충분하다. 여기서는 예제 파일 ch08 디렉터리 안에 있는 lorem.txt 파일을 열어보자. 다음처럼 사용한다.</p>
        <h5>코드8-1 파일 열기</h5>
        ...
        <p>첫 번째 파리미터는 현재 인터프리터가 실행 중인 경로 또는 스크립트가 실행 중인 경로를 기준ㅇ로 둔 파일의 상대경로이다. 두 번째는 파일을 여는 모드 옵션이다. 파일을 여는 모드옵션은 몇 가지가 있으며 open() 함수는 두 번째 파라미터가 없다면 옵션 'r'을 기본 값으로 적용한다. </p>
        <p>다음은 파라미터로 전달할 수 있는 모드 옵션이다.</p>
        <ul>
          <li><strong>'r'</strong> : 기본값, 읽기 전용으로 연다.</li>
          <li><strong>'w'</strong> : 파일 쓰기. 파일이 이미 존재한다면 해당 파일을 비운다.</li>
          <li><strong>'x'</strong> : 배타적 생성. 파일이 이미 존재한다면 open() 실행은 실패한다.</li>
          <li><strong>'a'</strong> : 파일 쓰기. 파일이 이미 존재한다면 파일의 끝에 내용을 덧붙인다.</li>
          <li><strong>'b'</strong> : 바이너리 모드로 연다.</li>
          <li><strong>'t'</strong> : 텍스트 모드로 연다.</li>
          <li><strong>'+'</strong> : 읽기와 쓰기를 다 한다.</li>
        </ul>
        <p>그리고 앞 모드 옵션들은 'w+b'(파일을 열고 0바이트로 비워버린다)라든지 'r+b'(파일을 바이너리 모드로 읽고 쓸 수 있게 연다)등으로 조합해서 사용할 수 있다. 또한 기본 값으로 설정된 모드 옵션들은 달리 대체할 수 있는 모드를 설정하지 않는 이상 언제나 붙는다. 즉, 아무런 파라미터도 전달하지 않으면 'r'이고 'r'은 'rt'와 같다.</p>

        <p>파일을 열었으면 읽어 볼 차례다. r, w로 열면 파일 커서는 처음에 있고, a 로 열면 파일의 마지막에 내용을 덧붙여야 하므로 파일의 커서는 마지막에 가 있다.</p>
      </article>
    </section>
    ...
    <section>
      <article>
        <p>파일 객체의 read() 함수는 파일 전체 내용을 몽땅 다 읽어온다.</p>
    ...
    <section>
      <article>
        <p>다시 파일을 열고 파일 안의 한 행을 읽어 보자. 이 때는 readline()을 사용한다.</p>
    ...
    <section>
      <article>
        <p>readlines()는 파일을 전부 읽어서 리스트 아이템으로 반환한다.</p>
    ...
    <section>
      <article>
        <p>file 객체는 순회(iterable)가능하므로 for 문에 리스트 대신 file 객체를 넣을 수 있다.</p>
        ...
        <p>for 문 안 리스트가 들어갈 자리에 file 객체가 들어간 것을 확인할 수 있다. 이렇게 하면 루프를 한 번 실행할 때마다 read()를 호출한 것처럼 동작할 수 있다.</p>
      </article>
    </section>
  </div>
  ...
  <div class="chapter">
    <section>
      <article>
        <p>파이썬도 파일을 열고 나면 닫아 주어야 한다. 그럴 땐 close()를 사용하면 된다. 닫고 나면 더 이상 해당 file 객체에 대해 작업할 수 없다.</p>
        <h5>코드8-7 파일 닫기</h5>
    ...
    <section>
      <article>
        <p>파이썬의 with 는 파일을 열고 닫는 과정을 자동으로 해주고 그 과정에서 오류가 발생하면 알아서 처리까지 해주므로 편리하다.</p>
        <p>기본적인 형태는 import를 사용하는 것과 비슷하다.</p>
        ...
        <p>파일을 열면서 바로 as 를 이용해 이름을 할당하면, with 아래 블록에서 해당 이름으로 파일에 접근할 수 있다. with를 벗어나면 자동으로 닫아준다.</p>
        <h5>코드8-8 with 이용하기</h5>
        ...
        <p>열고 닫는 과정을 with 구문이 전부 해결 해준다고 생각하고 with 안에서만 파일로 작업하면 된다.</p>
        <p>with 의 더 자세한 사용법은 <a href="http://effbot.org/zone/python-with-statement.htm" target="_blank">Understanding Python's "with" statement</a>나 파이썬 개발 문서의 <a href="https://www.python.org/dev/peps/pep-0343/" target="_blank">PEP 343 -- The "with" statement</a>를 참고한다.</p>
      </article>
    </section>
  </div>