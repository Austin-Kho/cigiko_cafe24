...
<div class="chapter">
  ...
  <section>
    <article>
      <p><code>SELECT</code> 문은 SQL 쿼리이다. 지금까지 설명한 <code>SELECT</code> 문은 하나의 데이터베이스 테이블에서 데이터를 가져오는 하나의 문으로, 아주 간단한 것이었다.</p>
  ...
  <section>
    <article>
      ...
      <p>이러한 각 단계를 각각의 쿼리로 실핼할 수도 있다. 즉 <code>SELECT</code> 문에서 반환한 결과를 다른 <code>SELECT</code> 문의 <code>WHERE</code> 절에 넣어 실행하는 방식을 반복하면 된다.</p>
      <p>하지만 하위 쿼리를 사용하면 이러한 세 쿼리를 하나의 문으로 압축할 수 있다.</p>
      <p>첫 번째 <code>SELECT</code> 문은 아주 간결하다. prod_id가 RGAN01인 주문 물품에 대한 order_num을 얻는 과정으로, 결과로 다음과 같이 두 개의 주문 번호를 얻을 수 있다.</p>
      ...
      <p>다음 단계는 주문 20007과 20008에 대한 고객 ID를 얻는 것이다. 5장에서 설명한 <code>IN</code>절을 사용해서 다음과 같이 <code>SELECT</code> 문을 만들 수 있다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT cust_id<br>
        FROM Orders<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;WHERE prod_id = 'RGAN01');
      ...
      <p>하위 쿼리는 항상 가장 안쪽에 있는 쿼리부터 바깥쪽 방향의 순서로 처리된다. 따라서 이 <code>SELECT</code> 문을 실행하면 DBMS 는 두 번의 연산을 하게 된다.</p>
      <p>첫 번째는 다음 하위쿼리를 실행하는 것이다.</p>
      <pre><code>SELECT order_num FROM OrderItems WHERE prod_id = 'RGAN01'</code></pre>
      <p>이 쿼리는 주문 번호인 20007과 20008을 반환하며, 이 두 값이 콤마로 구분된 형식으로 외부 쿼리의 <code>WHERE</code> 에 전달된다. 따라서 외부 쿼리는 다음과 같이 구성된다.</p>
      ...
      <p>고객 ID 를 직접 입력하는 대신 전에 만들어둔 <code>SELECT</code> 문을 하위 쿼리로 <code>WHERE</code> 절에 넣어보자.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT cust_name, cust_contact<br>
        FROM Customers<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 <code>SELECT</code> 문을 실행하기 위해서 DBMS는 실제로 세 개의 <code>SELECT</code> 문을 계산해야 한다. 가장 안쪽에 있는 하위 쿼리는 주문 번호 목록을 반환하고, 이 결과는 두 번째 쿼리의 <code>WHERE</code> 절에 사용되어 고객의 ID를 반환하며, 그 결과가 가장 바깥쪽에 있는 쿼리의 <code>WHERE</code> 절에 사용되기 때문이다. 원하는 데이터는 가장 바깥쪽에 있는 쿼리, 즉 최상위 커리에 의해 얻어진다.</p>
      <p>이와 같이 <code>WHERE</code> 절에 하위 쿼리를 사용하면 아주 강력하고 유연한 SQL 문을 작성할 수 있다. 중첩할 수 있는 하위 쿼리의 수에는 제한이 없으나, 실제로는 너무 복잡하게 중첩하는 것은 그다지 효율이 좋지 않다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      ...
      <ol>
        ...
        <p>앞서 두 단원에서 설명했듯이 <code>SELECT COUNT(*)</code>를 사용하면 테이블의 행을 셀 수 있으며 <code>WHERE</code> 절에 고객이 ID 를 넣으면 필터링이 가능할 것이다. 즉 1000000001이라는 고객의 주문을 세려면 다음과 같이 코드를 작성하면 된다.</p>
      </ol>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;WHERE Orders.cust_id = Customers.cust_id) AS orders<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 <code>SELECT</code> 문은 Customers 테이블에 있는 모든 고객에 대해 세 열을 반환한다. 첫 번째 고객의 이름인 cust_name, 두 번째는 고객의 상태를 나타내는 cust_state, 세 번째는 orders이다. orders는 괄호 내에 있는 하위 쿼리에 의해 만들어진 계산 필드로, 이 하위 쿼리는 얻어진 모든 고객에 대해 한 번씩 실행되어 각 고객의 주문 횟수를 반환하게 된다. 이 경우 하위 쿼리는 고객이 5명이므로 5번 실행된다.</p>
      <p>하위 쿼리의 <code>WHERE</code> 절은 지금까지 살펴본 것과 약간 다르다. 열 이름이 완전한 형식으로 지정되었기 때문이다. 이 부분은 Orders 테이블에서 cust_id를 가져와 현재 쿼리가 진행 중인(바깥쪽 쿼리) Customers 테이블의 cust_id와 비교하고, 이 값이 같은 경우를 세기 위한 것이다.</p>
  ...
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;WHERE cust_id = cust_id) AS orders<br>
      ...
      <p>하위 쿼리는 이러한 유형의 <code>SELECT</code> 문을 작성하는 데 매우 유용하지만 여러 테이블이 등장할 경우 열 이름 지정에 주의하지 않으면 엉뚱한 결과가 얻어질 수 있다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p>이 단원에서는 하위 쿼리가 무엇이며 어떻게 사용하는지 배웠다. 하위 쿼리의 가장 일반적인 쓰임은 <code>WHERE</code> 절 <code>IN</code> 연산자에 좋건을 넣기 위해서이며 계산 열을 채우는 데도 자주 사용된다. 두 가지 경우의 예를 모두 살펴보았으므로 꼼꼼히 익혀 두도록 하자.</p>
    </article>
  </section>
</div>