...
<div class="chapter">
  ...
  <section>
    <article>
      ...
      <p>이러한 모든 계산은 한 테이블에 있는 데이터, 또는 WHERE 절의 조건에 맞는 특정 데이터를 대상으로 수행되었다.</p>
  ...
  <section>
    <article>
      <p>그룹을 만들려면 SELECT 문에서 GROUP BY 절을 사용하면 된다. 이해를 위해 다음의 예를 보자.</p>
      ...
      <p>이 SELECT 문에는 두 개의 열인 vend_id 와 num_prods 가 지정되어 있다. vend_id 는 제품 공급업체의 ID이며 num_prods는 COUNT(*) 함수를 사용하는 계산 필드이다. GROUP BY 절은 데이터를 정렬하고 vend_id 를 기준으로 그룹화하도록 지시하므로, 전체 테이블이 아니라 vend_id 마다 num_prods 를 계산하게 된다. 즉, 각 공급업체별로 데이터를 그룹화하여 num_prods를 계산하는 것이다.</p>
      <p>GROUP BY 를 지정했기 때문에, 각 그룹별로 계산을 하라고 지정할 필요는 없다. 이는 자동으로 이루어진다. GROUP BY 절은 데이터를 이 절의 조건대로 그룹화 하여 전체 결과 집합이 아니라 각 그룹에 대해 집계를 하도록 지시한다.</p>
      <p>GROUP BY 를 사용하려면 먼저 다음과 같은 중요한 규칙을 이해해야 한다.</p>
      <ul>
        <li>GROUP BY 절에는 원하는 만큼의 열을 넣을 수 있다. 즉, 그룹을 중첩하는 등 데이터 그룹화를 보다 세부적으로 제어할 수 있다.</li>
        <li>GROUP BY 절에 중첩된 그룹이 있을 경우 데이터는 마지막 지정된 그룹을 기준으로 요약된다. 즉, 그룹화가 이루어질 때 모든 지정된 열이 함께 계산되며 각 열 수준의 데이터는 얻을 수 없다.</li>
        <li>GROUP BY 절에 나열되는 모든 열은 가져온 열이거나 올바른 식이어야 한다(집계 함수는 사용할 수 없다). SELECT 에 식을 사용하면 동일한 식이 GROUP BY 절에도 쓰여야 한다. 별칭은 사용할 수 없다.</li>
        <li>대부분의 SQL 에서는 GROUP BY 절에 길이 가변 형식(텍스트나 메모 필드와 같은)의 열을 지원하지 않는다.</li>
        <li>집계 계산 문을 제외하고, SELECT 문의 모든 열은 GROUP BY 절에 있어야 한다.</li>
        <li>그룹화 열에 NULL 값을 가진 행이 있을 경우, NULL 도 그룹으로 반환된다. NULL 값을 가진 행이 여러 개일 경우는 모두 하나로 그룹화 된다.</li>
        <li>GROUP BY 절은 모든 WHERE 절 뒤에 위치해야 하며 ORDER BY 절 앞에 위치해야 한다.</li>
      </ul>
  ...
  <section>
    <article>
      <p>GROUP BY 로 데이터를 그룹화 하는 것뿐만 아니라 어떤 그룹은 포함하고 어떤 그룹은 제외할지 지정하여 필터링 하는 것도 가능하다. 예를 들어 두 개 이상의 주문을 한 고객의 목록이 필요한 경우 각각의 행을 기준으로 필터링 하는 것보다 그룹으로 필터링 하는 것이 편하다.</p>

      <p>WHERE 절의 쓰임은 그 동안 많이 보았으며, 여기서 필요한 것도 그런 기능이다. 하지만 WHERE  절은 각 행에 대한 것이지 그룹에 대한 것이 아니므로 여기서는 사용할 수 없다.</p>
      <p>이 때 사용할 수 있는 것이 바로 HAVING 절이다. WHERE 절에 사용할 수 있는 모든 절은 HAVING 절에도 사용할 수 있으며, 유일한 차이점은 WHERE 절이 행을 대상으로 하는 대신 HAVING 은 그룹을 대상으로 한다는 점이다.</p>
      <div class="tip">
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>세 번째 줄까지의 내용은 별다른 것이 없으므로 마지막 HAVING 절만 살펴보자. 이 줄에서는 COUNT(*) >= 2라는 조건을 사용하여 두 개 이상의 주문을 한 고객만 선택하도록 하였다.</p>
      <p>이 필터링은 그룹 집계값을 대상으로 하며 각 행을 대상으로 하지 않기 때문에 WHERE 절은 사용할 수 없다.</p>
      <div class="tip">
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>첫 번째 줄은 집계 함수를 사용한 기본적인 SELECT 문으로, 지금까지의 예와 별반 다를 것이 없다. WHERE 절에서는 prod_price가 4 이상인 제품만을 필터링하고, GROUP BY 절에서 이 제품들을 vend_id 로 그룹화 한 다음 HAVING 절에서 이 그룹 중 제품의 개수가 2개 이상인 vend_id 만 골라낸다. 공급업체 DLL01의 경우 가격이 4 미만인 네 개의 제품을 판매하고 있으므로, WHERE 절이 없었다면 다음과 같이 조건에 맞지 않는 결과가 나타났을 것이다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT vend_id, COUNT(*) AS num_prods<br>
        FROM Products<br>
  ...
  <section>
    <article>
      <p>GROUP BY와 ORDER BY는 때로 같은 결과를 만들어내는 경우기 있지만 분명 다른 것임을 인식해야 한다. 차이점을 표 10.1 에 정리하였다.</p>
      ...
      <p>첫 번째 차이점은 매우 중요하다. GROUP BY에 의해 그룹화된 데이터가 그룹 순서로 정렬될 것이라 생각하지만 반드시 그렇지는 않다는 사실을 기억해야 한다. 대개의 DBMS 에서는 그룹 순서대로 정렬한다 해도 때로는 그룹 순서가 아니라 다른 순서대로 정렬해야 하는 경우도 생길 것이다. 데이터를 (특정한 집계값을 얻기 위해) 한 가지 방식으로 그룹화했다고 해서 결과가 같은 방식으로 정렬되어야 함을 의미하지는 않기 때문이다. 따라서 GROUP BY와 동일한 내용일지라도 정렬을 위해서는 ORDER BY를 분명히 써주어야 한다.</p>
      <div class="tip">        
      ...
      <p>GROUP BY와 ORDER BY의 쓰임을 설명하기 위해 예를 들어보자. 다음 SELECT 문은 이전 예와 비슷한 것으로, 세 개 이상의 물품이 포함된 모든 주문의 주문 번호와 물품 개수를 얻어낸다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
      ...
      <p>결과를 주문한 물품의 개수로 정렬하려면 다음과 같이 ORDER BY 절을 추가하면 된다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT order_num, COUNT(*) AS items<br>
        FROM OrderItems<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 예에서는 GROUP BY 절을 사용하여 주문 번호(order_num 열)로 데이터를 그룹화하고 COUNT(*)를 사용하여 각 주문의 물품 수량을 반환하였다. HAVING 절은 그룹화된 각 주문 중에서 수량이 3개 이상인 것만을 피러링하는 데 사용되었으며, 마지막으로 ORDER BY 절을 사용하여 결과를 정렬하였다.</p>
    </article>
  </section>
  ...
  <section>
    <article>
      <p>지금까지 SELECT 문에 넣었던 여러 절의 지정 순서를 정리해보자. 다음 표와 같이 각 절은 지정된 순서대로 입력되어야 한다.</p>
  ...
  <section>
    <article>
      <p>9장 '데이터 요약'에서 SQL 집계 함수를 사용하여 다양한 데이터 계산을 하는 방법을 배웠다. 이 단원에서는 GROUP BY 절을 사용하여 데이터를 그룹화 한 다음 각 그룹에 대해 계산을 하는 방법을 설명하였으며, HAVING 절을 통해 원하는 그룹만 필터링 하는 방법도 살펴보았다. ORDER BY와 GROUP BY의 차이점, 그리고 WHERE와 HAVING의 차이점도 설명하였다.</p>
    </article>
  </section>
</div>