...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ListCount := SQL%ROWCOUNT;<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 저장 프로시저는 ListCount 라는 해나의 매개변수만 취한다. 값을 저장 프로시저에 전달하는 대신 이 매개변수는 저장 프로시저로부터 값을 가져오는데, OUT 이라는 키워드가 이 특셩을 설명하고 있다. Oracle은 IN과 OUT, 그리고 INOUT이라는 매개변수 유형을 지원하는데, IN은 저장 프로시저로 값을 전달하는 것이고 OUT은 저장 프로시저에서 값을 가져오는 것이며 INOUT은 저장 프로시저에 값을 전달하면서 가져오기도 하는 것이다. 저장 프로시저 코드는 BEGIN과 END 문을 통해 시작과 끝이 지정되어 있으며 전자 메일 주소가 있는 고객을 가져오기 위한 간단한 SELECT 문이 실행되고 있다. 계산된 값은 ListCount에 저장되어 우리에게 반환된다.</p>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 저장 프로시저 역시 Orders 테이블에 새 주문을 추가하기 위한 것이지만 이번에는 DBMS 자체적으로 주문 번호를 만들도록 했다. 대부분의 DBMS 는 이러한 기능을 지원하며 SQL Server 는 이를 식별 필드로 사용되는 자동 증가 열이라고 한다. 다른 DBMS 에서는 자동 번호나 시퀀스 등으로 다양하게 부른다. 이번에도 하나의 매개변수인 고객 ID만 전달되었으며, 주문 번호와 주문 날짜는 모두 DBMS 에서 자동으로 계산하여 사용하였다. 자동으로 생성된 ID의 값이 무엇인지는 어떻게 알 수 있을까? SQL Server에서는 이를 위해 <code>@IDENTITY</code>라는 전역변수를 지원하므로 이 예에서와 같이 SELECT 문에서 이 변수를 사용하면 생성된 ID의 값을 반환할 수 있다.</p>
