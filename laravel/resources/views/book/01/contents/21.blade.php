...
<div class="chapter">
  ...
  <section>
    <article>
      ...
      <p>지난 18개 단원에서 계속 예로 사용해온 <code>Orders</code> 테이블을 생각해보자. 실제 주문은 <code>Orders</code> 테이블에, 주문한 항목은 <code>OrderItems</code> 테이블에 각각 저정되며 이 두 테이블은 기본 키라난 고유한 ID 를 사용하여 서로 연결되어 있다. 또한 이러한 테이블은 다시 고객이나 제춤 정보를 담고 있는 다른 테이블과 연결된다.</p>
      ...
      <ol>
        ...
        <li>고객의 ID를 얻는다.</li>
        <li>고객 ID와 연결된 행을 <code>Orders</code> 테이블에 추가한다.</li>
        <li>Orders 테이블에 할당된 새 주문 ID를 얻는다.</li>
        <li>주문된 각 항목에 대해 하나씩의 행을 <code>OrderItems</code> 테이블에 추가하고 가져온 ID를 사용하여 <code>Orders</code> 테이블과 연결한다(<code>Products</code> 테이블과는 제품 ID로 연결한다).</li>
      </ol>

      <p>이러한 작업을 수행하는 도중에 디스크 용량 부족, 보안 제한 사항, 테이블 잠금 등과 같은 데이터베이스 오류가 발생하여 전체 작업이 모두 완료되지 않았다면 어떻게 될까?</p>

      <p>만약 고객이 추가되고 <code>Orders</code> 테이블이 추가되기 전에 문제가 발생했다면 큰 오류는 없을 것이다. 주문 내역이 없는 새 고객이 추가된 정도의 결과이기 때문이다. 나중에 다시 이 작업을 수행하면 미리 삽입된 고객 정보가 그대로 사용될 수 있다.</p>

      <p>그러나 <code>Orders</code> 테이블에 행이 추가된 후, <code>OrderItems</code> 행이 추가되기 전에 오류가 발생이 되면 어떻게 될까? 주문은 주문인데 주문 항목이 없는 주문된 항목 정보가 없는 빈 주문이 데이터베이스에 남게 된다.</p>

      <p><code>OrderItems</code> 행을 추가하는 도중에 오류가 발생하면 상황은 더 나빠진다. 주문의 일부 내역만 데이터베이스에 남게 될 것이고, 이 정보가 잘못된 것인지 조차 알 수가 없다.</p>
      ...
      <ol>
        ...
        <li>고객의 ID 를 얻는다.</li>
        <li><code>Orders</code> 테이블에 행을 추가한다.</li>
        <li><code>Orders</code> 에 행을 추가하는 중 오류가 발생하면 롤백한다.</li>
        <li>Orders 테이블에 할당된 새 주문 ID를 가져온다.</li>
        <li>주문된 각 항목 당 하나의 행을 <code>OrderItems</code> 테이블에 추가한다.</li>
        <li><code>OrderItems</code> 테이블에 행을 추가하는 중 오류가 발생하면 추가된 <code>OrderItems</code> 및 <code>Orders</code> 행을 취소하고 롤백한다.</li>
      </ol>
  ...
  <section>
    <article>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 예에서는 주문 번호 <code>12345</code>가 시스템에서 완전히 삭제된다. 이 트랜잭션은 <code>Orders</code> 와 <code>OrderItems</code> 라는 두 개의 테이블에 관련되어 있는데, 트랜잭션이 사용된 것은 두 테이블 중 하나에서만 삭제되는 부분적인 삭제가 이루어지지 않도록 하기 위해서이다. 마지막 <code>COMMIT</code> 문에 이르기까지 아무런 오류 없이 작업이 진행되었다면 이 문에 작업이 커밋되고, 지정한 내용이 완전히 삭제된다. 물론 도중에 오류가 발생했다면 커밋되지 않고 롤백된다.</p>
  ...
  <section>
    <article>
      ...
      <p>예를 들어, 앞서 설명한 주문 추가 작업은 하나의 트랜잭션이지만 오류가 발생하면 <code>Orders</code> 행이 추가되기 전의 시점으로 되돌려야 하며, 만약 그 전에 <code>Customers</code> 테이블에 추가 작업을 헸다면 여기까지는 되돌리고 싶지 않을 것이다.</p>
