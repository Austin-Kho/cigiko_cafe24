...
<div class="chapter">
  ...
  <section>
    <article>
      ...
      <p>뷰를 이해하는 가장 좋은 방법은 예를 보는 것이다. 12장 '테이블 조인'에서 소개했던 다음과 같은 SELECT 문을 떠올려보자. 세 개의 테이블에서 데이터를 가져오는 SELECT 문의 예이다.</p>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;AND prod_id = 'RGAN01';
      ...
      <p>이 쿼리는 특정한 제품을 주문한 고객의 정보를 얻기 위한 것으로, 이 데이터를 얻으려면 테이블 구조와 쿼리 작성 방법, 그리고 테이블 조인 방법 역시 알아야 한다. 또한 다른 제품(또는 여러 제품)에 대한 동일한 데이터를 얻으려면 마지막 WHERE 절을 수행해야 한다.</p>
      <p>이러한 전체 쿼리를 ProductCustomers 라는 가상 테이블로 묶는다고 가정해보자. 이제 SELECT 문이 보다 간결해진다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT cust_name, cust_contact<br>
  ...
  <section>
    <article>
      ...
      <p>대부분의 경우 일단 뷰를 만들어두면 테이블과 같은 방법으로 사용할 수 있다. SELECT 문을 수행하거나 필터링 또는 정렬도 가능하고, 다른 뷰나 테이블과 조인하거나 데이터 추가 및 업데이트도 가능하다. 추가와 업데이트의 경우 약간의 제한이 있는데 이에 대해서는 다시 설명할 것이다.</p>
  ...
  <section>
    <article>
      ...
      <ul>
        ...
        <li>뷰 쿼리에서 ORDER BY 절을 사용하는 것은 많은 DBMS에서 제한된다.</li>
        ...
        <li>일부 DBMS에서는 특정 행에 삽입이나 업데이트 작업을 했을 때 이로 인해 뷰에서 이 행이 제외되는 상황이 발생할 경우 이러한 삽입이나 업데이트 작업을 할 수 없도록 제한하고 있다. 예를 들어 전자 메일 주소가 있는 고객만을 가져오는 뷰가 있을 때 특정한 고객의 전자 메일 주소를 지우도록 업데이트하면 이 행은 뷰에서 가져온ㄴ 조건에 맞지 않게 되므로 이러한 작업을 할 수 없다. 대부분의 DBMS 에서는 이러한 작업도 가능하도록 허용하지만 그렇지 않은 경우도 있으니 주의하자.</li>
      </ul>
      <div class="tip">
        <h4><span class="badge badge-secondary">TIP</span> 각 DBMS의 설명서를 참고하자.</h4>
        <p>관련된 규칙이 많고 또 각 DBMS에서 제한하는 특별한 규칙들이 있을 것이므로 뷰를 만들기 전에 각 DBMS의 서령서를 읽어 보는 것이 필수적이ㅏ.</p>
      </div>
    </article>
  </section>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;AND OrderItems.order_num = Orders.order_num;
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 문은 ProductCustomers라는 이름의 뷰를 만든다. 이 뷰는 세 개의 테이블을 조인하여 제품을 주문한 모든 고객의 목록을 반환한다. SELECT * FROM ProductCustomers 를 실행하면 원하는 목록을 얻을 수 있다.</p>
      <div class="tip">
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 문은 WHERE 절을 실행하여 특정한 뷰를 통해 데이터를 가져온다. DBMS에서 이 요청이 실행되면 뷰 쿼리에 이미 있던 WHERE 절이 이 WHERE 절을 추가하여 데이터가 올바르게 필터링 되도록 조정한다.</p>
      <p>이와 같이 뷰를 사용하면 복잡한 SQL 문을 아주 간단하게 표현할 수 있다. 정확히 말하자면 복잡하게 만들어두고 뷰로 감추는 것이다. 일단 감춰진 내용은 간한한 뷰를 통해 편리하게 사용될 수 있다. 뷰를 사용하면 기반이 되는 SQL 을 한 번 작성해두고 필요할 때마다 다시 사용할 수 있는 것이다.</p>
  ...
  <section>
    <article>
      <p>앞서 설명했듯이 뷰의 또 다른 쓰임 중 하나는 가져온 데이터의 포맷팅을 다시 지정하는 것이다. 7장에서 소개했던 다음 SELECT 문을 살펴보자. 이 SELECT 문은 결합된 하나의 계산 열을 사용해서 공급업체의 이름과 위치를 반환한다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 문은 앞서 설명한 SELECT 문과 동일한 쿼리를 만든다. 만약 우편물 발송을 위해 모든 공급업체의 주소가 필요하다면 다음과 같이 실행하면 된다.</p>
  ...
  <section>
    <article>
      <p>뷰는 일반적인 WHERE 절을 적용하는 데도 유용하다. 예를 들어 CustomerEMailList라는 뷰를 정의하여 전자 메일 주소가 없는 모든 고객을 걸러내려면 다음과 같은 문을 만들면 된다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>메일 목록에 있는 사용자에게 전자 메일을 보낸다면 분명 전자 메일 주소가 없는 사용자는 메일 목록에서 제외해야 할 것이다. 이 WHERE 절이 바로 그러한 역할을 한다. cust_email 열에 NULL 값이 있는 행을 모두 걸러내어 선택되지 않도록 하는 것이다.</p>
  ...
  <section>
    <article>
      <p>뷰는 계산 필드의 사용을 단순화할 때도 아주 유용하다. 7장에서 설명했던 다음과 같은 SELECT 문을 떠올려보자. 이 문은 특정한 번호의 주문 항목을 가져와 총 금액을 계산한다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT prod_id,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;quantity,<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;quantity*item_price AS expanded_price<br>
  ...
  <section>
    <article>
      <p>뷰는 가상 테이블이다. 뷰는 데이터를 포함하지 않으며 데이터를 가져오는 데 필요한 쿼리만 포함한다. 뷰는 SQL SELECT 문에 대한 일종의 캡슐화이며 데이터 작업을 단순화하고 기반 데이터의 포맷팅 재지정이나 보안에도 응용될 수 있다.</p>
    </article>
  </section>
</div>