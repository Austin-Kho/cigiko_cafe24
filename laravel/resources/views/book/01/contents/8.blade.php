...
<div class="chapter">
  ...
  <section>
    <article>
      ...
      <p>이런 모든 경우, 응용 프로그램에서 필요한 형식과 데이터베이스 테이블에 저장되어 있는 형식이 다르기 때문에 무언가 조치가 필요하다. 이럴 때 계산 필드가 유용하다. 지금까지 살펴본 열과는 달리, 계산 필드는 실제로 데이터베이스에 존재하는 것은 아니며 SQL SELECT 문 내에서 곧바로 생성되는 것이다.</p>

      <blockquote><strong>필드</strong>: 열과 같은 의미이며 어떤 것을 사용해도 무방하다. 하지만 대개 데이터베이스 열을 열이라 부르며, 필드는 계산 필드를 사용할 때 흔히 부르는 용어이다.</blockquote>

      <p>SELECT 문 중에 어떤 열이 실제 테이블 열이고 어떤 열이 계산된 필드인지는 데이터베이스밖에 모른다. 클라이언트 입장에서는 계산 필드 역시 어떠한 방식으로든 데이터베이스에서 가져온 데이터로 보일 뿐이다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      ...
      <p>SELECT 문으로 공급업체의 이름과 위치를 얻는 것은 쉽지만 이 값을 어떻게 결합해야 할까?</p>
      <blockquote><strong>결합</strong>: 하나의 긴 값을 만들기 위해 값을 결합하는 것</blockquote>
      <p>해결 방법은 두 열을 결합하는 것이다. SQL SELECT 문에는 이를 위한 특별한 연산자가 있다. 이 연산자는 DBMS 에 따라 다르며, <code>+</code> 기호나 <code>||</code> 기호가 사용된다.</p>
      ...
      <p>같은 문이지만 || 기호를 사용하는 경우는 다음과 같다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT RTRIM(vend_name) + ' (' || RTRIM(vend_country) || ' )'<br>
        FROM Vendors<br>
      ...
      <p>이 예에서 SELECT 문은 다음 요소를 결합한다.</p>
      <ul>
      ...
      <p>결과를 통해 알 수 있듯이 SELECT 문은 결합된 하나의 열을 반환하며, 이 열에는 네 개의 요소가 하나로 결합되어 있다. RTRIM()함수는 값의 오른쪽에 있는 모든 공백을 잘라낸다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p>결과를 통해 알 수 있듯이 주소 필드를 결합하는데 SELECT 문은 별 문제 없이 사용되었다. 그렇다면 새로운 이름의 계산 열을 만들려면 어떻게 해야 할까? 사실 이름이란 것은 있을 수 업 수 없고 값만 있을 뿐이다. SQL 쿼리 도구에서 결과로 볼 때는 이렇게 해도 관계 없겠지만 클라이언트 응용 프로그램에서는 이 열을 참조할 방법이 필요하므로 아무래도 이름이 있어야 한다.</p>

      <p>이 문제를 해결하려면 SQL이 지원하는 열 별칭을 사용하면 된다. 별칭이란 필드나 값의 데체 이름으로, <code>AS</code> 키워드를 사용해 지정할 수 있다. 다음 SELECT 문을 살펴보자.</p>
    <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
    <pre><code>
    ...
    <h4><span class="badge badge-pill badge-info">분 석</span></h4>
    <p>SELECT 문 자체는 전에 살펴본 코드와 동일하지만 뒤에 <code>AS</code> 키워드가 있고 새로운 이름이 븥어있다는 사실만 다르다. 이는 계산된 필드에 vend_title 이라는 이름을 붙이라는 의미로, 출력 결과에서 볼 수 있듯이 결과는 동일하지만 열의 이름이 vend_title 인 것을 알 수 있다.</p>
