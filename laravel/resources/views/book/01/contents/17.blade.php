...
<div class="chapter">
  <section>
    <article>
      <p>이 단원에서는 UPDATE와 DELETE 문을 사용하여 테이블 데이터를 제어하는 방법에 대해 알아보자.</p>
    </article>
  </section>
</div>
...
<div class="chapter">
  ...
  <section>
    <article>
      <p>테이블의 데이터를 업데이트(변경)할 때는 UPDATE 문을 사용하며, UPDATE 문은 다음과 같은 두 가지 목적으로 사용된다.</p>
      <ul>
      ...
      <p>UPDATE 문의 사용 방법은 아주 간단하다. 기본 형식은 다음과 같은 세 부분으로 이루어진다.</p>
      <ul>
        <li>업데이트할 테이블</li>
        <li>열 이름과 새 값</li>
      ...
      <p>UPDATE 문은 항상 업데이트할 테이블 이름으로 시작한 다음 SET 명령을 사용하여 없데이트할 열과 새 값을 지정한다. 마무리는 WHERE 절에서 어떤 행을 업데이트할지 지정한다. 만약 WHERE 절을 지정하지 않는다면 모든 고객의 전자메일 주소가 kim@thetoystore.com으로 업데이트 되므로 원하지 않는 결과가 발생할 것이다.</p>
      ...
      <p>이와 같이 여러 열을 업데이트하려면 SET 명령 내에 '열 이름 = 값' 부분을 콤마로 구분하여 여러 번 적어주면 된다(마지막 열 뒤에는 콤마를 적지 않는다).</p>
      <div class="tip">
      ...
      <p>열의 값을 삭제하려면 NULL로 설정하면 된다. 물론 NULL을 허용하도록 테이블이 정의되어 있는 경우에만 가능하다. 구문은 다음과 같다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        UPDATE Customers<br>
      ...
      <p>이렇게 하면 cust_email 열의 값이 NULL 로 설정된다.</p>
    </article>
  </section>
  ...
  <section>
    <article>
      <p>테이블에서 데이터를 삭제하려면 DELETE 문을 사용한다. DELETE 문은 다음 과 같은 두 가지 방법으로 사용할 수 있다.</p>
      <ul>
      ...
      <p>UPDATE 문은 사용하기 쉽다고 설명했었다. DELETE 문은 이보다 더 쉽다.</p>
      ...
      <p>이 문의 쓰임을 쉽게 이해할 수 있을 것이다. DELETE FROM 은 어떤 테이블의 데이터를 삭제할 것인지 지정하는 부분이고, WHERE 절은 어떤 행을 삭제할지 지정하는 부분이다. 만약 WHERE 절을 지정하지 않으면 Customers 테이블의 모든 행이 삭제된다.</p>
      <div class="tip">
      ...
      <p>DELETE 에는 열 이름이나 와일드카드 문자를 사용할 수 없으며 열이 아닌 전체 행을 삭제한다. 특정한 열을 삭제하려면 UPDATE 문을 사용해서 NULL로 설정해 주어야 한다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p>지금까지 소개한 UPDATE와 DELETE 문에는 모두 WHERE 절이 있었으며, 이렇게 하는 데에는 분명한 이유가 있다. WHERE 절을 생략하면 UPDATE나 DELETE는 테이블의 모든 행에 적용될 것이다. 즉 테이블의 모든 행을 업데이트하거나 삭제하게 되기 때문이다.</p>
      <p>일반적으로 SQL 프로그래머가 따라야 하는 지침은 다음과 같다.</p>
      <ul>
        <li>전체 행을 업데이트하거나 삭제하려는 경우가 아니면 절대 WHERE 절이 없는 채로 UPDATE 나 DELETE 문을 실행하지 않는다.</li>
        <li>테이블에 기본 키가 있는 지 확인하고(기본키에 대해서는 12장을 참조하기 바란다), 만약 있다면 이를 WHERE 절의 조건으로 사용하자. 각 기본 키를 직접 지정하거나, 기본 키를 지정하거나 값의 범위를 지정할 수도 있다.</li>
        <li>UPDATE 나 DELETE 문에서 WHERE 절을 사용하기 전에 WHERE 절을 통해 필터링되는 대상이 정확한지 확인하기 위해 SELECT 문을 사용해보자. 잘못된 WHERE 절로 UPDATE 나 DELETE 문을 실행한 뒤에는 돌이킬 수 없다.</li>
        <li>데이터베이스에서 지정하는 참조 무결성을 사용하자(12장 참조). 이렇게 하면 업데이트하거나 지우는 행이 다른 테이블에 연결되어 있을 경우 데이터베이스의 무결성 규칙에 의해 제지되므로 보다 안전하다.</li>
        <li>어떤 DBMS 에서는 데이터베이스 관리자가 WHERE 절이 없는 UPDATE 나 DELETE 문의 실행을 막을 수 있는 규칙을 만들 수도 있다.이러한 기능이 지원되는지 확인하고, 필요하다면 사용해보자.</li>
      </ul><br>
      <div class="tip">
  ...
  <section>
    <article>
      <p>이 단원에서는 UPDATE와 DELETE 문을 사용하여 테이블의 데이터를 제어하는 방법을 배웠다. 각 문의 구문과 잠재된 위험성을 알아보았으며, UPDATE와 DELETE 문에서 WHERE 절이 중요한 이유도 설명하였다. 마지막으로 데이터가 유실되는 상황을 막기 위해 일반적으로 따라야 하는 지침에 대해 살펴보았다.</p>
    </article>
  </section>
</div>