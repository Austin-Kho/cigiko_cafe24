...
<div class="chapter">
  ...
  <section>
    <article>
      ...
      <p>프로그래밍 방식으로 테이블을 만들기 위해서는 CREATE TABLE이라는 SQL 문을 사용한다. 대화식 도구를 사용하는 경우도 사실은 SQL문이 만들어지는 것이며, 실제 작업은 SQL문을 통해 이루어진다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p>CREATE TABLE 문을 사용하여 테이블을 만들려면 다음 정보를 지정해야 한다.</p>
      <ul>
        <li>만들 새 테이블의 이름을 CREATE TABLE 뒤에 지정한다.</li>
...
&nbsp;&nbsp;&nbsp;prod_desc   VARCHAR(1000)  NULL<br>
      ...
      <p>이 구문에서 알 수 있듯이 테이블 이름은 CREATE TABLE 키워드 바로 뒤에 지정되며, 테이블 정의(모든 열)는 괄호 내에 위치하고 열 사이는 콤마로 구분된다. 이 테이블의 경우 다섯 개의 열로 구성되어 있는데, 각 열은 열 이름(테이블 내에서 고유해야 함)으로 시작하여 열의 데이터 형식(데이터 형식에 대해서는 1장을, 자주 사용되는 데이터 형식과 호환성에 대해서는 4장 참조)이 이어진다. 전체 문을 종료할 때는 세미콜론(;)으로 마무리 한다.</p>
      <p>CREATE TABLE 의 구문은 DBMS에 따라 다르다며 이와 같은 간단한 문에서도 이를 증명할 수 있다. 이 문은 Oracle, PostgreSQL, SQL Server, Sybase에서는 잘 작동하지만 MySQL의 경우는 VARCHAR를 text로 바꾸어야 하고, DB2의 경우는 마지막 열의 NULL을 삭제해야 한다. 각 DBMS에 대한 SQL 테이블 생성 스크립트 예를 살펴보라고 한 이유가 바로 이 때문이다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p>NULL 값을 혀용하는 열은 이 열에 아무런 값을 넣지 않아도 된다는 의미이다. NULL 값을 혀용하지 않는 열일 경우 값이 없는 행은 받아들여지지 않으므로 행을 추가하거나 업데이트할 때 반드시 값을 넣어야 한다.</p>
      <p>모든 열은 NULL열이거나 NOT NULL열이므로 이 상태는 테이블을 만들 때 테이블 정의에 지정해 주어야 한다. 다음 예를 보자.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
...
&nbsp;&nbsp;&nbsp;cust_id      CHAR(10)   NOT NULL<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 문은 이 책에서 샘플로 사용하는 Orders 테이블을 만드는 문으로, 주문 번호, 주문 날짜, 그리고 고객의 ID를 포함하는 세 개의 열이 지정되었다. 세 열 모두 반드시 필요하므로 NOT NULL로 지정되었는데, 이렇게 하면 이러한 값이 지정되지 않은 열이 삽입되는 것을 방지할 수 있다. 만약 값을 지정하지 않고 행을 추가하려하면 오류가 발생하고 삽입은 실패하게 된다.</p>

      <p>다음 예는 NULL과 NOT NULL 열을 혼합하여 사용한 경우이다.</p>
...
&nbsp;&nbsp;&nbsp;vend_country    CHAR(50)<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 문 역시 이 책에서 샘플로 사용하는 Vendors 테이블을 만드는 것으로, 공급업체의 ID와 이름은 반드시 필요하기 때문에 NOT NULL로 지정되었지만 나머지 다섯 개의 열은 굳이 값을 지정하지 않아도 되기 때문에 NULL을 허용하도록 지정되었다. NULL이 기본값이기 때문에 이와 같이 NULL이나 NOT NULL 중 하나를 지정하지 않으면 NULL을 지정한 것과 마찬가지로 인식된다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p>행을 삽입할 때 값을 지정하지 않으면 기본으로 사용될 값이 기본값이며, SQL에서 이를 지정할 수 있다. 기본값은 CREATE TABLE 문의 열 정의 부분에서 DEFAULT 키워드를 사용하여 지정할 수 있다.</p>
      <p>다음 예를 살펴보자.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
...
&nbsp;&nbsp;&nbsp;item_price    DECIMAL(8, 2)    NOT NULL<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 문은 주문을 구성하는 개별 항목을 포함하는 OrderItems라는 테이블을 만드는 문이다(주문 자체는 Orders 테이블에 저장된다). quantity 열은 주문한 각 향목의 수량을 의미하는데, DEFAULT 1이라고 지정함으로써 기본 값을 넣었다. 즉 주문 시 수량을 지정하지 않으면 수량 1개를 사용하도록 지정한 것이다.</p>

      <p>기본값은 날짜나 타임스탬프 열에 값을 저장할 때도 종종 사용된다. 예를 들어 함수나 변수를 사용하여 날짜를 시스템 날짜로 지정할 수 있다. MySQL 사용자는 DEFAULT CURRENT_DATE()를, Oracle 사용자는 DEFAULT SYSDATE를, SQL Server사용자는 DEFAULT GETDATE()를 사용하면 현재 시스템 날짜를 기본값으로 지정할 수 있다. 시스템 날짜를 얻기 위한 명령이 DBMS마다 다르므로 다음 표를 참조하여 사용하기 바란다.</p>
      <h5>표 17.1 시스템 날짜 얻기</h5>
      <table class="table table-hover tavle-sm">
        <thead>
          <tr>
            <th>DBMS</th>
            <th>함수/변수</th>
  ...
  <section>
    <article>
      ...
      <ul>
        ...
        <li>기존 테이블에 열을 추가하는 것은 모든 DBMS에서 허용되지만 추가할 수 었는 데이터 형식과 NULL 및 DEFAULT 의 사용에 대해서는 약간의 제한이 있다.</li>
      ...
      <p>ALTER TABLE 문을 사용하여 테이블을 변경하려면 다음 정보를 지정해야 한다.</p>
      <ul>
        <li>변경할 테이블의 이름을 ALTER TABLE 키워드 뒤에 지정해야 한다. 반드시 이미 존재하는 테이블이어야 하며 그렇지 않으면 오류가 발생한다.</li>
      ...
      <ul>
        ...
        <li>15장에서 배운 INSERT SELECT 문을 사용하여 이전 테이블에서 새 테이블로 데이터를 복사한다. 필요할 경우 변환 함수나 계산 필드도 사용한다.</li>
  ...
  <h3 class="sub-header"><svg id="i-file" viewBox="0 0 32 32" width="20" height="20" fill="none" stroke="currentcolor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
    ...
    <section>
      <article>
        <p>테이블의 이름을 바꾸는 방법은 DBMS 마다 다르며 쉽고 빠르게 설명하기 힘들다. DB2, MySQL, Oracle, PostgreSQL 사용자는 RENAME 문을 사용하면 되고, SQL Server와 Sybase 사용자는 기본 제공되는 sp_rename 저장 프로시저를 사용하면 된다.</p>
  ...
  <section>
    <article>
      <p>이 단원에서는 새로운 몇 가지 SQL 문에 대해 배웠다. 새 테이블을 만들 때는 CREATE TABLE 문이 사용되며 기존 테이블의 열(또는 제약 조건이나 인덱스와 같은 다른 개체)을 변경할 때는 ALTER TABLE 문을, 테이블을 완전히 삭제할 때는 DROP TABLE 문을 사용하면 된다. 이러한 문은 모두 데이터베이스에 큰 영향을 주는 것이므로 반드시 주의하여 사용하여야 하며, 가급적이면 중요한 데이터를 모두 백업한 후에 실행하는 것이 좋다. 각 문의 정확한 구문은 각각의 DBMS 사용 설명서를 참고한다.</p>
    </article>
  </section>
</div>