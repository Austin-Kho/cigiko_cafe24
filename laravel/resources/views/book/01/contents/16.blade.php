...
<div class="chapter">
  <section>
    <article>
      <p>이 단원에서는 SQL <code>INSERT</code> 문을 사용하여 테이블에 데이터를 삽입하는 방법에 대해 알아보자.</p>
    </article>
  </section>
</div>
...
<div class="chapter">
  ...
  <section>
    <article>
      <p><code>SELECT</code> 는 의심할 여지없이 가장 자주 사용되는 SQL 문이며, 지금까지의 14개 단원에서 <code>SELECT</code> 에 대해 설명한 이유도 바로 그것 때문이다. 하지만 이외에도 자주 사용되는 SQL문이 있으며 이러한 문에 대해서도 잘 알아두어야 한다. 그 중 첫 번째 문은 <code>INSERT</code>로, 이 단원과 다음 단원에 걸처 알아볼 것이다.</p>
      <p>이름이 말해 주듯이 <code>INSERT</code>는 데이터베이스 테이블에 행을 삽입(추가)하는 데 사용된다. <code>INSERT</code>가 사용되는 용도는 몇 가지가 있다.</p>
      <ul>
        <li>하나의 완전한 행을 삽입</li>
  ...
  <section>
    <article>
      <p>테이블에 데이터를 삽입하는 가장 간단한 방법은 기본적인 <code>INSERT</code> 구문을 사용하는 것으로, 테이블 이름과 함께 여기에 삽입할 새 행의 값을 입력하면 된다.<br>예를 들어보자.</p>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NULL);
      ...
      <p>이 예에서는 Customers 테이블에 새 고객 정보를 삽입한다. 각 테이블 열에 저장될 값은 <code>VALUES</code> 절에 지정되어 있으며 모든 열의 값을 지정해야 한다. 모든 열의 값을 지정해야 한다. 이 예에서와 같이 cust_contact나 cust_email 처럼 넣을 값이 없는 행에는 <code>NULL</code> 값을 사용해야 한다(물론 이렇게 하려면 <code>NULL</code> 값이 허용되도록 테이블이 구성되어 있어야 한다). 열은 테이블 정의에 지정된 순서대로 채워진다.</p>
      <div class="tip">
      ...
      <p>약간은 귀찮지만 보다 안전한 방법은 다음과 같은 방식으로 <code>INSERT</code> 문을 작성하는 것이다.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NULL);
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 예의 실행 결과는 앞서 살펴본 <code>INSERT</code> 문과 정확히 동일하지만, 이번에는 테이블 이름 뒤에 괄호를 넣고 열 이름을 명확하게 지정해 주었다. 이 행이 DBMS에 삽입될 때는 여기에 지정된 열 목록에 따라 적절해<code> VALUES</code> 목록의 값이 삽입된다. <code>VALUES</code> 절의 첫 번째 값은 첫 번째 지정된 열 이름에 대응되고, 두 번째 값은 두 번째 지정된 열 이름에 대응되는 식으로 열 이름과 값이 짝지어 삽입되는 것이다.</p>
      <p>열 이름이 지정되었으므로 <code>VALUES</code>에 지정된 값은 해당하는 열 이름에 맞게 테이블에 삽입되며, 실제로 테이블의 열 순서가 어떻게 구성되었는지에 영향을 받지 않는다. 이렇게 하면 테이블의 열 순서가 변경되더라도 <code>INSERT</code> 문을 그대로 사용할 수 있다는 장점이 있다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p>앞서 설명했듯이 <code>INSERT</code> 문을 사용하는 권장되는 방법은 테이블 열 이름을 정확하게 지정해준ㄴ 것이다. 이 방법을 사용하면 일부 열에 값을 지정하지 않을 수도 있다. 즉 일부 열에만 값을 지정하고 나머지 열에는 값을 지정하지 않아도 된다.<br>다음 예를 보자.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        INSERT INTO Customers(cust_id,<br>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;cust_name,<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 예를 보면 cust_contact 와 cust_email은 열 목록에 넣지 않았고 값도 지정하지 않았다. 따라서 이 <code>INSERT</code> 문은 값을 넣을 때 이 두 값을 제외한 채로 넣게 된다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p><code>INSERT</code>는 대개 지정된 값을 사용하여 테이블에 행을 추가할 때 쓰이지만 <code>SELECT</code> 문의 결과를 데이터에 삽입하는 것도 가능하며 이 때는 구문이 약간 다르다. 이를 <code>INSERT SELECT</code>라고 하며 이름이 말해 주듯이 <code>INSERT</code> 문과 <code>SELECT</code> 문이 결합된 형태이다.</p>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;cust_country<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 예 <code>INSERT SELECT</code> 문을 사용하여 CustNew 테이블의 데이터를 Customers 테이블에 삽입한다. 이렇게 삽입문을 사용하면 삽입할 값을 VALUES 를 사용하여 일일이 지정할 필요없이 CustNew 테이블의 데이터를 가져와 삽입할 수 있다. <code>SELECT</code> 내의 각 열은 앞서 지정된 열 목록과 대응되어야 한다. 이 문을 실행하면 CustNew 테이블에 있는 모든 데이터가 Customers 테이블에 삽입된다.</p>
      <div class="tip">
      ...
      <p><code>INSERT SELECT</code> 내에 사용되는 <code>SELECT</code> 문에 <code>WHERE</code> 절을 넣으면 삽입할 데이터를 필터링할 수 있다.</p>
      <div class="tip">
  ...
  <section>
    <article>
      <p><code>INSERT</code> 문을 사용하지 않는 방식의 데이터 삽입 작업도 있다. 한 테이블의 내용을 새로운 다른 테이블(즉석에서 만들어지는)에 삽입하려면 <code>SELECT INTO</code>문을 사용한다.</p>
      <div class="tip">
        <h4><span class="badge badge-secondary">참고</span> DB2에서는 지원되지 않는다.</h4>
        <p>DB2는 SELECT INTO 문을 지원하지 않는다.</p>
      </div>
      <p><code>INSERT SELECT</code> 는 기존 테이블에 데이터를 추가하지만 <code>SELECT INTO</code> 는 데이터를 새 테이블에 복사해 넣는다. DBMS 의 특성에 따라 다르지만 테이블이 이미 있을 경우 덮어쓰는 경우도 있다.</p>
      <div class="tip">
      ...
      <p><code>SELECT INTO</code>의 사용 예를 보자.</p>
      <h4><span class="badge badge-pill badge-primary">입 력</span></h4>
      <pre><code>
        SELECT *<br>
        INTO CustCopy<br>
      ...
      <h4><span class="badge badge-pill badge-info">분 석</span></h4>
      <p>이 <code>SELECT</code> 문은 CustCopy라는 새 테이블을 만들고 Customers테이블의 모든 내용을 복사하여 이 테이블에 삽입한다. <code>SELECT *</code> 라고 지정되었으므로 Customers 테이블의 모든 열이 CustCopy 테이블에 만들어지고 복사될 것이다. 열 중에서 일부만 복사하려면 와일드카드문자인 <code>*</code> 를 빼고 열 이름을 직접 지정하면 된다.</p>
      ...
      <p><code>SELECT INTO</code> 를 사용할 때 고려해야 할 점이 있다.</p>
      <ul>
        <li><code>WHERE</code> 이나 <code>GROUP BY</code> 등과 같은 모든 <code>SELECT</code> 옵션을 선택할 수 있다.</li>
  ...
  <section>
    <article>
      <p>이 단원에서는 <code>INSERT</code> 문을 사용하여 데이터베이스 테이블에 행을 추가하는 방법을 배웠다. <code>INSERT</code>를 사용하는 여러 가지 방법, 열을 직접 지정하는 방식이 좋은 이유, <code>INSERT SELECT</code> 를 사용하여 다른 테이블에서 행을 가져오는 방법, <code>SELECT INTO</code> 를 사용하여 새 테이블에 행을 내보내는 방법을 살펴보았다. 다음 단원에서는 <code>UPDATE</code>와 <code>DELETE</code>를 사용하여 테이블 데이터를 제어하는 방법을 알아보도록 하자.</p>
    </article>
  </section>
</div>