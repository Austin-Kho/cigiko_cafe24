    <h2 class="page-header">{{$sub[$id][1]}}</h2>
    <div class="chapter">
      <section>
        <article class="">
          <p>파이썬 프로그래밍 언어는 광범위한 구문 구조, 표준 라이브러리 함수, 대화형 개발 환경 기능을 갖추고 있다. 다행히 그 중 대부분은 무시해도 된다. 모두를 알아야 할 필요는 없지만 몇 가지 기본적인 프로그래밍 개념은 배워야 할 것이다.</p>
          <p>대화형 쉘을 사용해서 예제들을 따라 직접 시도해 보자. 그저 읽은 것보다는 해보는 것이 훨씬 기억이 잘 된다.</p>
          <ul>
            <li><a href="/book01/2">{{$sub[2][1]}}</a></li>
            <li><a href="/book01/3">{{$sub[3][1]}}</a></li>
            <li><a href="/book01/4">{{$sub[4][1]}}</a></li>
            <li><a href="/book01/5">{{$sub[5][1]}}</a></li>
            <li><a href="/book01/6">{{$sub[6][1]}}</a></li>
            <li><a href="/book01/7">{{$sub[7][1]}}</a></li>
          </ul>
        </article>
      </section>
    </div>
