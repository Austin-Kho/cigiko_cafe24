...
class Cm3 extends CI_Controller {
	...
	public function project($mdi='', $sdi=''){
		...
		$this->load->view('/cms_views/menu/cm3/project_v', $menu);
		...
		if($mdi==1 && $sdi==1 ){
			...
			if( !$auth['_m3_1_1'] or $auth['_m3_1_1']==0) { // 조회 권한이 없는 경우
				...
				if($this->form_validation->run() == FALSE) {
					...
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm3/md1_sd1_v', $data);
					...
					if($this->input->post('mode')=='reg'){ // 데이터 등록 모드
						...
						alert('정상적으로 프로젝트 데이터 정보가 등록 되었습니다.', base_url('cm3/project/1/1')."?mode=".$this->input->post('mode')."&amp;new_pj=".$this->input->post('new_pj')."&amp;end_pj=".$this->input->post('end_pj'));
					}
				}
			}
		...
		// 프로젝트 관리 2. 기본정보 수정 ////////////////////////////////////////////////////////////////////
			...
			if( !$auth['_m3_1_2'] or $auth['_m3_1_2']==0) {
				...
				if($this->form_validation->run() == FALSE) { // 폼 전송 데이타가 없으면,
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm3/md1_sd2_v', $data);
				}else{
					//폼 데이타 가공
					...
					if($result) { // 등록 성공 시
						...
						alert('데이터베이스 오류가 발생하였습니다..', base_url('cm3/project/1/2/'));
						exit;
					}
				}
			}
		...
		// 신규 프로젝트 1. 신규등록 ////////////////////////////////////////////////////////////////////
			...
			if( !$auth['_m3_2_1'] or $auth['_m3_2_1']==0) {
				...
				if($this->form_validation->run() == FALSE) { // 폼 전송 데이타가 없으면,
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm3/md2_sd1_v', $data);
				}else{
					//폼 데이타 가공
					$local_addr = $this->input->post('postcode1')."|".$this->input->post('address1_1')."|".$this->input->post('address2_1');
					$type_name = $this->input->post('type_name_1', TRUE);
					...
					if($result) { // 등록 성공 시
						...
						alert('데이터베이스 오류가 발생하였습니다..', base_url('cm3/project/2/1/'));
						exit;
					}
				}
			}
		...
		// 신규 프로젝트 1. 예비검토 ////////////////////////////////////////////////////////////////////
			...
			if( !$auth['_m3_2_2'] or $auth['_m3_2_2']==0) {
				...
				//본 페이지 로딩
				$this->load->view('/cms_views/menu/cm3/md2_sd2_v', $data);
			}
		}
	}
}
