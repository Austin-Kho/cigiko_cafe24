...
class Cm5 extends CI_Controller {
	 ...
	 */
	 public function config($mdi='', $sdi=''){
		...
		$this->load->view('/cms_views/menu/cm5/config_v', $menu);
		...
		if($mdi==1 && $sdi==1 ){
			...
			if( !$auth['_m5_1_1'] or $auth['_m5_1_1']==0) { // 조회 권한이 없는 경우
				...
				//페이지네이션 설정/////////////////////////////////
				$config['base_url'] = base_url('cm5/config/1/1/');   //페이징 주소
				$config['total_rows'] = $this->cm5_m->com_div_list($div_table, '', '', $st1, $st2, 'num');  //게시물의 전체 갯수
				...
				// db[전체부서목록] 데이터 불러오기
				$data['all_div'] = $this->cm5_m->all_div_name($div_table);

				//  db [부서]데이터 불러오기
				$data['list'] = $this->cm5_m->com_div_list($div_table, $start, $limit, $st1, $st2, '');

				// 세부 부서데이터 - 열람(수정)모드일 경우 해당 키 값 가져오기
				if($this->input->get('seq')) $data['sel_div'] = $this->cm5_m->select_data_row($div_table, $where = array('seq' => $this->input->get('seq')));
				...
				if($this->form_validation->run()==FALSE) {
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm5/md1_sd1_v', $data);
				}else{
					$div_data = array(
					...
					if($this->input->post('mode')=='reg') {
						$result = $this->cm5_m->insert_data($div_table, $div_data);
					}else if($this->input->post('mode')=='modify') {
						$result = $this->cm5_m->update_data($div_table, $div_data, $where = array('seq' => $this->input->post('seq')));
					}else if($this->input->post('mode')=='del') {
						$result = $this->cm5_m->delete_data($div_table, $this->input->post('seq'));
					}
					if($result){
						alert('정상적으로 처리되었습니다.', base_url('cm5/config/1/1/'));
					}else{
						alert('다시 시도하여 주십시요.', base_url('cm5/config/1/1/'));
					}
				}
			}
		...
		// 1. 기본정보관리 2. 직원관리 ////////////////////////////////////////////////////////////////////
			...
			if( !$auth['_m5_1_2'] or $auth['_m5_1_2']==0) {
				...
				//페이지네이션 설정/////////////////////////////////
				$config['base_url'] = base_url('cm5/config/1/2/');  //페이징 주소
				$config['total_rows'] = $this->cm5_m->com_mem_list($mem_table, '', '', $st1, $st2, 'num');  //게시물의 전체 갯수
				$config['per_page'] = 10; // 한 페이지에 표시할 게시물 수
				$config['num_links'] = 3; // 링크 좌우로 보여질 페이지 수
				$config['uri_segment'] = 5; //페이지 번호가 위치한 세그먼트
				$config['reuse_query_string'] = TRUE; //http://example.com/index.php/test/page/20?query=search%term
				...
				// db[전체부서목록] 데이터 불러오기
				$data['all_div'] = $this->cm5_m->all_div_name('cms_com_div');

				//  db [직원 ]데이터 불러오기
				$data['list'] = $this->cm5_m->com_mem_list($mem_table, $start, $limit, $st1, $st2, '');

				// 세부 부서데이터 - 열람(수정)모드일 경우 해당 키 값 가져오기
				if($this->input->get('seq')) $data['sel_mem'] = $this->cm5_m->select_data_row($mem_table, $where = array('seq' => $this->input->get('seq')));
				...
				if($this->form_validation->run()==FALSE) {
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm5/md1_sd2_v', $data);
				}else{
					...
					if($this->input->post('mode')=='reg') {
						$result = $this->cm5_m->insert_data($mem_table, $mem_data);
					}else if($this->input->post('mode')=='modify') {
						$result = $this->cm5_m->update_data($mem_table, $mem_data, $where = array('seq' => $this->input->post('seq')));
					}else if($this->input->post('mode')=='del') {
						$result = $this->cm5_m->delete_data($mem_table, $this->input->post('seq'));
					}
					if($result){
						alert('정상적으로 처리되었습니다.', base_url('cm5/config/1/2/'));
					}else{
						alert('다시 시도하여 주십시요.', base_url('cm5/config/1/2/'));
					}
				}
			}
		...
		// 1. 기본정보관리 3. 거래처정보 ////////////////////////////////////////////////////////////////////
			...
			if( !$auth['_m5_1_3'] or $auth['_m5_1_3']==0) {
				...
				//페이지네이션 설정/////////////////////////////////
				$config['base_url'] = base_url('cm5/config/1/3/');  //페이징 주소
				$config['total_rows'] = $this->cm5_m->com_accounts_list($acc_table, '', '', $st1, $st2, 'num');  //게시물의 전체 갯수
				$config['per_page'] = 10; // 한 페이지에 표시할 게시물 수
				$config['num_links'] = 3; // 링크 좌우로 보여질 페이지 수
				$config['uri_segment'] = 5; //페이지 번호가 위치한 세그먼트
				$config['reuse_query_string'] = TRUE; //http://example.com/index.php/test/page/20?query=search%term
				...
				//  db [거래처 ]데이터 불러오기
				$data['list'] = $this->cm5_m->com_accounts_list($acc_table, $start, $limit, $st1, $st2, '');

				// 세부 거래처데이터 - 열람(수정)모드일 경우 해당 키 값 가져오기
				if($this->input->get('seq')) $data['sel_acc'] = $this->cm5_m->select_data_row($acc_table, $where = array('seq' => $this->input->get('seq')));
				...
				if($this->form_validation->run()==FALSE) {
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm5/md1_sd3_v', $data);
				}else{
					...
					if($this->input->post('mode')=='reg') {
						$result = $this->cm5_m->insert_data($acc_table, $acc_data);
					}else if($this->input->post('mode')=='modify') {
						$result = $this->cm5_m->update_data($acc_table, $acc_data, $where = array('seq' => $this->input->post('seq')));
					}else if($this->input->post('mode')=='del') {
						$result = $this->cm5_m->delete_data($acc_table, $where = array('seq' => $this->input->post('seq')));
					}
					if($result){
						alert('정상적으로 처리되었습니다.', base_url('cm5/config/1/3/'));
					}else{
						alert('다시 시도하여 주십시요.', base_url('cm5/config/1/3/'));
					}
				}
			}
		...
		// 1. 기본정보관리 4. 계좌관리 ////////////////////////////////////////////////////////////////////
			...
			if( !$auth['_m5_1_4'] or $auth['_m5_1_4']==0) {
				...
				//페이지네이션 설정/////////////////////////////////
				$config['base_url'] = base_url('cm5/config/1/4/');  //페이징 주소
				$config['total_rows'] = $this->cm5_m->bank_account_list($bank_table, '', '', $st1, $st2, 'num');  //게시물의 전체 갯수
				$config['per_page'] = 10; // 한 페이지에 표시할 게시물 수
				$config['num_links'] = 3; // 링크 좌우로 보여질 페이지 수
				$config['uri_segment'] = 5; //페이지 번호가 위치한 세그먼트
				$config['reuse_query_string'] = TRUE; //http://example.com/index.php/test/page/20?query=search%term
				...
				// db[전체은행목록] 데이터 불러오기
				$data['com_bank'] = $this->cm5_m->all_bank_name();
				//은행 디비 전체 불러오기
				$data['all_bank'] = $this->cm5_m->select_data_list('cms_capital_bank_code');
				$data['all_div'] = $this->cm5_m->select_data_list('cms_com_div');

				//  db [은행 ]데이터 불러오기
				$data['list'] = $this->cm5_m->bank_account_list($bank_table, $start, $limit, $st1, $st2, '');

				// 세부 은행데이터 - 열람(수정)모드일 경우 해당 키 값 가져오기
				if($this->input->get('seq')) $data['sel_bank'] = $this->cm5_m->select_data_row($bank_table, $where = array('no' => $this->input->get('seq')));
				...
				if($this->form_validation->run()==FALSE) {
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm5/md1_sd4_v', $data);
				}else{
					$bank_name = $this->cm5_m->select_data_row('cms_capital_bank_code', $where = array('bank_code' => $this->input->post('bank_code')));
					$bank_data = array(
					...
					if($this->input->post('mode')=='reg') {
						$result = $this->cm5_m->insert_data($bank_table, $bank_data);
					}else if($this->input->post('mode')=='modify') {
						$result = $this->cm5_m->update_data($bank_table, $bank_data, $where = array('no' => $this->input->post('seq')));
					}else if($this->input->post('mode')=='del') {
						$result = $this->cm5_m->delete_data($bank_table, $where = array('no' => $this->input->post('seq')));
					}
					if($result){
						alert('정상적으로 처리되었습니다.', base_url('cm5/config/1/4/'));
					}else{
						alert('다시 시도하여 주십시요.', base_url('cm5/config/1/4/'));
					}
				}
			}
		...
		// 2. 회사정보관리 1. 회사정보 ////////////////////////////////////////////////////////////////////
			...
			if( !$auth['_m5_2_1'] or $auth['_m5_2_1']==0) { // 조회권한 없을 때
				...
				if($this->form_validation->run() == FALSE) { // 폼 전송 데이타가 없으면,
					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm5/md2_sd1_v', $data); // 조회권한 있고 폼 전송 데이타가 없을 때 등록권한 데이터 및 등록데이터와 함께 폼 열기
				}else{
					//폼 데이타 가공
				...
				if($data['mode']=='com_reg') {
					$result = $this->cm5_m->insert_data('cms_com_info', $com_data);
					$msg = '등록';
				}else if($data['mode']=='com_modify') {
					$result = $this->cm5_m->update_data('cms_com_info', $com_data, array('seq'=>1));
					$msg = '변경';
				}

				if($result) {
					// 등록 성공 시
					alert('회사 정보가 '.$msg.' 되었습니다.', base_url('cm5/config/2/1/'));
					exit;
				}else{ // 등록 실패 시
					// 실패 시
					alert('회사 정보'.$msg.'에 실패하였습니다.\n 다시 시도하여 주십시요.', base_url('cm5/config/2/1/'));
					exit;
				}
			}
		}
		...
		// 2. 회사정보관리 2. 권한관리 ////////////////////////////////////////////////////////////////////
			...
			if(( !$auth['_m5_2_2'] or $auth['_m5_2_2']==0) && $this->session->userdata['is_admin']!=1) {
				...
				if($this->form_validation->run() == FALSE) { // 폼 전송 데이타가 없으면,
					...
					$data['new_rq'] = $this->cm5_m->new_rq_chk();   //  신규 등록 신청자가 있는 지 확인
					$data['user_list'] = $this->cm5_m->user_list(); // 승인된 유저 목록
					$data['sel_user'] = $this->cm5_m->sel_user($this->input->get('un', TRUE)); //  선택된 유저 데이터
					$data['user_auth'] = $this->cm5_m->user_auth($this->input->get('un', TRUE)); //  선택된 유저의 권한 데이터

					//본 페이지 로딩
					$this->load->view('/cms_views/menu/cm5/md2_sd2_v', $data);
				}else{
					...
					if($this->input->post('no')){
						...
						$result = $this->cm5_m->rq_perm($where_no, $auth_data);
						if($result){
							alert('요청하신 작업이 정상적으로 처리 되었습니다.', base_url('cm5/config/2/2/'));
							exit;
						}else{
							alert('데이터베이스 에러입니다. 다시 확인하여 주십시요', base_url('/cm5/config/2/2/'));
							exit;
						}
						//사용자 승인//////////////////////////////////////////////
					}
					...
					if($this->input->get('un')&&$this->input->post('user_no')&&$this->input->post('user_id')){
						...
						$auth_result = $this->cm5_m->auth_reg($this->input->get('un'), $auth_dt);
						if($auth_result) alert('요청하신 작업이 정상적으로 처리되었습니다.', base_url('cm5/config/2/2/')."?un=".$this->input->get('un')); else alert('데이터베이스 에러입니다. 다시 시도하여 주십시요.', base_url('cm5/config/2/2/'));
					}
