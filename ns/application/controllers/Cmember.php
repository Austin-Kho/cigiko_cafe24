...
class cms_member extends CI_Controller
{
	...
	public function login(){
		...
		if($this->form_validation->run() == FALSE) { // 폼 전송 데이타가 없으면,
			...
			// 일단 유저 데이터를 불러온다
			$user = $this->cmem_m->user_chk($this->input->post('user_data', TRUE));
	...
	public function join() {
		...
		if($this->form_validation->run() == FALSE) { // 폼 전송 데이타가 없으면,
			...
			$result = $this->cmem_m->join($join_data);
	...
	public function modify() {
		...
		$data['user'] = $this->cmem_m->user_chk($this->session->userdata('user_id'));
		...
		if($this->form_validation->run() == FALSE) { // 폼 전송 데이타가 없으으면,
			...
			if( !empty($data['user']) && password_verify($this->input->post('passwd'), $data['user']->passwd)){
				...
				$result = $this->cmem_m->modify($modify_data);
