...
<?php endforeach; ?>
		...
		<div class="col-xs-12 table-responsive" style="padding: 0;">
			<table class="table table-bordered table-hover table-condensed">
				...
				<tbody>
<?php
$tp = $this->cmain_m->sql_row("SELECT type_name, type_color FROM cms_project WHERE seq='$project' ");
if(!empty($tp)) :
...
foreach($rec_list as $lt) :
	...
	$contractor = $this->cmain_m->sql_row(" SELECT contractor AS ct FROM cms_sales_contractor WHERE cont_seq='$lt->cont_seq' ");
	$total_rec = $this->cmain_m->sql_row(" SELECT SUM(paid_amount) AS pa FROM cms_sales_received WHERE pj_seq='$project' AND cms_sales_received.cont_seq='$lt->cont_seq' GROUP BY cms_sales_received.cont_seq ");
?>
					<tr class="center">
