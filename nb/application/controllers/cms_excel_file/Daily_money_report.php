...
class Daily_money_report extends CI_Controller {
	...
	public function excel_file(){
		...
		// 은행계좌 데이터
		$bank_acc = $this->cm4_m->select_data_lt('cb_cms_capital_bank_account', '', '', '');
		...
		// 조합 대여금 데이터
		$jh_data = $this->cm4_m->select_data_lt('cb_cms_capital_cash_book', 'any_jh', 'any_jh<>0', 'any_jh');
		...
		// 설정일 입금 내역
		$da_in = $this->cm4_m->select_data_lt("cb_cms_capital_cash_book", "account, cont, acc, inc, note", "(com_div>0 AND class2<>8) AND (class1='1' or class1='3') AND deal_date='".$sh_date."'", "", "seq_num");
		// 설정일까지 입금 내역
		$da_in_total = $this->cm4_m->da_in_total('cb_cms_capital_cash_book', $sh_date);
		// 설정일 출금내역
		$da_ex = $this->cm4_m->select_data_lt("cb_cms_capital_cash_book", "account, cont, acc, exp, note", "(com_div>0) AND (class1='2' or class1='3') AND deal_date='".$sh_date."'", "", "seq_num");
		// 설정일까지 출금내역
		$da_ex_total = $this->cm4_m->da_ex_total('cb_cms_capital_cash_book', $sh_date);
