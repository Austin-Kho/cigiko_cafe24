...
class Contract_data extends CI_Controller {
	...
	public function excel_file(){
		...
		//
		// if($sh_frm['sh_text']){
		// 	if($sh_frm['sh_con']==0) $where.=" AND (account like '%".$sh_frm['sh_text']."%' OR cont like '%".$sh_frm['sh_text']."%' OR acc like '%".$sh_frm['sh_text']."%' OR evidence like '%".$sh_frm['sh_text']."%' OR cb_cb_cms_capital_cash_book.worker like '%".$sh_frm['sh_text']."%') "; // 통합검색
		...
		// 검색결과 데이터 --------------------------------------------------------//
		// $list_num = $this->m4_m->cash_book_list('cb_cms_capital_cash_book, cb_cms_capital_bank_account', $where, '', '', $sh_frm, 'num', '');
		// $cb_list = $this->m4_m->cash_book_list('cb_cms_capital_cash_book, cb_cms_capital_bank_account', $where, '', '', $sh_frm, '', 'ex');
		// 검색결과 데이터 --------------------------------------------------------//
		...
		if($cb_list) :
			...
			// $b_acc = $this->cms_main_model->sql_result(' SELECT no, name FROM cb_cms_capital_bank_account ORDER BY no ');
