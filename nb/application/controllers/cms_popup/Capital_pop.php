...
class Capital_pop extends CI_Controller
{
	...
	public function cash_book($id)
	{
		...
		$data['row'] = $this->cms_main_model->select_data_row('cms_capital_cash_book', $where);

		// 계정별 세부계정과목 구하기
		$data['acnt1'] = $this->cms_main_model->sql_result(" SELECT * FROM cms_capital_account_d3 WHERE d1_code='1' AND is_sp_acc !='1' ORDER BY d3_code ASC ");
		$data['acnt2'] = $this->cms_main_model->sql_result(" SELECT * FROM cms_capital_account_d3 WHERE d1_code='2' AND is_sp_acc !='1' ORDER BY d3_code ASC ");
		$data['acnt3'] = $this->cms_main_model->sql_result(" SELECT * FROM cms_capital_account_d3 WHERE d1_code='3' AND is_sp_acc !='1' ORDER BY d3_code ASC ");
		$data['acnt4'] = $this->cms_main_model->sql_result(" SELECT * FROM cms_capital_account_d3 WHERE d1_code='4' AND is_sp_acc !='1' ORDER BY d3_code ASC ");
		$data['acnt5'] = $this->cms_main_model->sql_result(" SELECT * FROM cms_capital_account_d3 WHERE d1_code='5' AND is_sp_acc !='1' ORDER BY d3_code ASC ");

		// 현장목록 가져오기
		$data['pj'] = $this->cms_main_model->sql_result(" SELECT seq, pj_name FROM cms_project WHERE is_end!='1' ORDER BY biz_start_ym DESC, seq DESC ");

		// 입출금 계좌 가져오기 select * from cms_capital_bank_account
		$data['bank_acc'] =  $this->cms_main_model->sql_result(" select * from cms_capital_bank_account ");
		...
		if($this->form_validation->run()==FALSE) {
			...
			$result = $this->cms_main_model->update_data('cms_capital_cash_book', $deal_data, $where);
