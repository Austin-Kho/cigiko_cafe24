...
class Project_data extends CI_Controller
{
	...
	public function data_modi ($pj, $seq) {
		...
		// 프로젝트 리스트 정보
		$data['all_pj'] = $this->cms_main_model->sql_result(' SELECT * FROM cms_project  ORDER BY biz_start_ym DESC ');
		$data['now_pj'] = $this->cms_main_model->sql_row(" SELECT * FROM cms_project  WHERE seq='".$pj."'");
		$data['type'] = explode("-", $data['now_pj']->type_name);

		// 수정할 데이터 정보
		$data['modi_data'] = $this->cms_main_model->sql_row(" SELECT * FROM cms_project_all_housing_unit WHERE seq='".$seq."'" );
		// 동 리스트 정보
		$data['dong'] = $this->cms_main_model->sql_result(" SELECT dong FROM cms_project_all_housing_unit GROUP BY dong ORDER BY dong " );
		...
		if($this->form_validation->run() == FALSE) {
			...
			$result = $this->cms_main_model->update_data('cms_project_all_housing_unit', $update_data, $where = array('seq' => $this->input->post('seq')));
