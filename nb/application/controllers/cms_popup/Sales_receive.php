...
class Sales_receive extends CI_Controller
{
	...
	public function index(){
		...
		if( !$auth['_m1_2_1'] or $auth['_m1_2_1']==0) {
			...
			// 납부 회차 데이터
			$pay_sche = $data['pay_sche'] = $this->cms_main_model->sql_result( "SELECT seq, pay_name FROM  cms_sales_pay_sche WHERE pj_seq='$project' ORDER BY pay_code " );

			// price seq 전체 가져오기
			$price = $data['price'] = $this->cms_main_model->sql_result(" SELECT seq, unit_price FROM cms_sales_price WHERE pj_seq='$project' ORDER BY seq ");

			//계약 통계 계산
			$data['total_sum'] = 0;
			$data['sche_sum'] = 0;
			for ($i=0; $i<count($price); $i++) {
				$cont_sum = $this->cms_main_model->sql_row(" SELECT COUNT(seq) AS num FROM cms_sales_contract WHERE price_seq='".$price[$i]->seq."' ");
