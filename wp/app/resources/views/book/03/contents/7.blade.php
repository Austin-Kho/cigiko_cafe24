  ...
  <div class="chapter">
    <section>
      <article>
        ...
        <p>위의 예제처럼 '<code>class 클래스이름:</code>'을 입력한 다음 행부터 클래스에 속할 변수와 함수를 작성하면 된다. 아무것도 없이 일단 클래스 이름부터 정해두고 싶다면 위 예제와 같이 <code>pass</code>만 적어 두어도 클래스의 조건은 다 갖춘 셈이다.</p>
  ...
  <div class="chapter">
    <section>
      <article>
        ...
        <blockquote>__init__()은 파이썬의 클래스 생성자(Constructor)이다. 클래스를 생성하고 초기화 하는 순간 무언가 할 수 있도록 마련된 순서가 __init__()인 셈이다. __init__은 self를 파라미터로 전달하는데 이는 __init__()이 실행되는 시점에 이미 인스턴스의 생성이 끝났다는 것을 암시한다. 덕분에 self를 이용해서 인스턴스 변수를 설정할 수 있다.</blockquote>
        <div class="tip"><h4>NOTE_ 초기화와 클래스 인스턴스 생성</h4><p>__init__()함수에 self라는 파라미터를 전달한다는 점에서 초기화하는 시점에서 이미 클래스 인스턴스의 생성은 완료되어 있다. __init__()함수는 말 그대로 초기화를 한다.</p><p>정말로 초기화하는 순간에 무언가를 하고 싶다면 파이썬 문서의 '<a href="https://docs.python.org/3/reference/datamodel.html#basic-customization" target="_blank">Basic customization</a>'을 참고한다.</p></div>
  ...
  <div class="chapter">
    <section>
      <article>
        ...
        <p>하지만 일반적인 함수와는 달리 클래스 메서드의 첫 번째 파라미터는 언제나 클래스 자신을 참조하는 self로 지정해야 한다. 이 변수는 외부에서 호출할 때 영향을 주지 않는다. 이 규칙은 앞에서 먼저 마주친 <code>__init__()</code>에도 적용된다.</p>
        <div class="tip"><h4>NOTE_ self 를 명시적으로 전달해야 하는 이유</h4><p>왜 self 를 명시적으로 전달하는지는 파이썬을 만든 귀도 반 로섬이 직접 쓴 '<a href="http://neopythonic.blogspot.kr/2008/10/why-explicit-self-has-to-stay.html" target="_blank">Why explicit self has to stay</a>'라는 글을 참고하면 좋다.</p><p>물론 함수의 파리미터니 self를 다른 이름으로 변경할 수 있다. 자바 개발자들에게 친숙한 this 등으로 말이다. 하지만 이는 (강력하게) 권장하지 않는다.</p></div>
        ...
        <p>이제 <code>Diva</code> 클래스 인스턴스들은 <code>song()</code>과 <code>medley()</code> 메서드를 사용할 수 있다.</p>
        <h5>코드6-6 <code>song()</code>과 <code>medley()</code> 메서드 실행</h5>
        ...
        <p>인스턴스가 아니라 클래스에서 직접 메서드를 호출했다는 것을 눈여겨 본다. <code>self</code> 자리에 <code>Diva</code> 인스턴스를 전달하면 해당 인스턴스로 메서드를 호출한 것처럼 작동한다.</p>
        <p>물론 인스턴스를 생성하지 않고도 해당 클래스의 메서드를 호출하도록 만들 수 있다. <code>self</code> 를 메서드의 파라미터로 추가 하지 않는 것이다.</p>
  ...
  <div class="chapter">
    <section>
      <article>
        ...
        <div class="tip"><h4>NOTE_ 파이썬3의 object 클래스</h4>
        <p>파이썬3의 모든 클래스는 암시적으로 object 클래스를 상속받는다. 2.7버전까지만 해도 파이썬 클래스를 만드는 가장 기본적인 코드는 다음과 같았다.</p>
        <h5>코드6-9 파이썬 2.7의 클래스 정의</h5>
        <pre class="python"><code><blockquote><ol><li>Class&nbsp;Heroes<font>&#40;</font><font color="#008000">object</font><font>&#41;</font>:</li><li>&nbsp;&nbsp;&nbsp;&nbsp;<font color="#ff7700">pass</font></li></ol></blockquote></code></pre>
        <p>앞 클래스는 명시적으로 object 를 상속받아서 새로운 Heroes 클래스를 만든 것이다.</p></div>
        <p>앞에서 만든 <code>Diva</code> 클래스를 상속 받아서 무언가를 새로 만들어 본다.</p>
        ...
        <blockquote><strong>TIP</strong>: <code>__init__()</code>은 인스턴스 생성이 아니라 초기화를 담당하므로 [코드6-10]처럼 <code>super.__init__()</code>이라고 정의하지 않으면 슈퍼클래스가 초기화되지 않는다.</blockquote>
