					...
					<div style=" height:18px; background-color:#F8F8F8" class="d3_sub">
						<b><font size="2" color="#cc0099">◈</font><font size="2" color="#6666cc"> <?=$s_di_2?></font></b>
					...
					<!-- =====subject table end===== -->
					<?
						$cg_1_2_rlt = mysql_query("select cg_1_2 from cms_mem_auth where user_id='$_SESSION[p_id]' ", $connect);
						$cg_1_2_row = mysql_fetch_array($cg_1_2_rlt);

						if(!$cg_1_2_row[cg_1_2]||$cg_1_2_row[cg_1_2]==0){
					?>
					...
					<tr>
						...
						<tr>
							<td height="538" style="width:200px; border-width:1px 1px 1px 0; border-style:solid; border-color:#cccccc; background-color:#f8f8f8; padding:5px 5px 5px 5px;" valign="top">
								<div style="height:50px; margin-bottom:6px; background-color:#f0e8e8;"></div>
								...
								<div style="height: 25px; padding:5px 5px 0 5px; margin:0 5px 0 5px;" class="bottom">
									<a href="<?$_SERVER['PHP_SELF']?>?m_di=1&amp;s_di=2" style="color:#000000;"><b>직원정보 관리</b></a>
								</div>
								<div style="height: 25px; padding:5px 5px 0 5px; margin:0 5px 0 5px;" class="bottom">
								...
								<div style="height:435px;">
									<?
										...
										if(!$ss_di||$ss_di==1){// 초기 화면이나 리스트 불러올 때
											include "basic2_1.php";
										}else if($ss_di==2){ // 신규 수정등록 화면일 때
											include "basic2_2.php";
										}
									?>
								</div>
							</td>
