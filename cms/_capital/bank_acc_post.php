	...
	if($mode=="del"){
		...
		if($mode=="reg"){
			...
			if($total_num){            // 같은 정보가 있을 때 이전 페이지로 옮김
				...
				$query="INSERT INTO `cms_capital_bank_account` ( `bank_code`, `bank`, `name` ,`number` , `is_com`, `div_seq`, `pj_seq`, `holder`, `acc_seq`, `acc_cms_code` , `open_date` , `note`)
																		VALUES('$bank_code', '$bank', '$name', '$number', '$is_com', '$com_div', '$pj_seq', '$holder', '$acc_seq', '$acc_cms_code', '$open_date', '$note')";
				$result=mysql_query($query, $connect);
