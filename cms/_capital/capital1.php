					...
					<tr>
						...
						<div style="height:18px; text-align:right; padding:0 20px 2px 0; margin-top:10px;" class="form2">
							<a href="javascript:" onClick="excel_pop(<?=$ca_1_1_row[ca_1_1]?>,2);"><img src="../images/excel_icon.jpg" height="10" border="0" alt="" /> EXCEL 출력</a>
						</div>
						...
						<tr align="center">
							...
							<tr bgcolor="#f2f2f9">
								...
								<b><font color="#ee0066">▶</font> <font color="#003399">자 금 현 황</font></b>
								</td>
								<td align="right" style="padding:0 10px 0 0px;border-width: 1px 1px 1px 0; border-color:#E1E1E1; border-style: solid;" height="28">(단위 : 원)</td>
							</tr>
							<tr bgcolor="#f5f5f5">
								...
								<td align="center" style="padding:0 0 0 10px;border-width: 0 1px 1px 0; border-color:#E1E1E1; border-style: solid;">금일잔액</td>
							</tr>
							<?
									...
									for($i=0; $i<=$num; $i++){
										 ...
										 if($d_rows[no]==1){ // 은행 계좌 테이블의 오토어그리먼트 1번은 현금계정을 기본 설정으로 한 경우.(프로그램 세팅 시 해당 테이블의 1번 항목을 현금계정으로 기본 입력 할 것)
												$td_str="<td align='center' style='padding:0 0 0 10px;border-width: 0 0 1px 0; border-color:#E1E1E1; border-style: solid;'>현금</td>";
										 }else if($d_rows[no]==2&&$d_rows[name]<>'현금'){
												$td_str="<td align='center' style='padding:0 0 0 10px;border-width: 0 0 1px 0; border-color:#E1E1E1; border-style: solid;' rowspan='$num'>보통예금</td>";
										 }else{
												$td_str="";
										 }
							...
							<tr>
								...
								<td align="right" style="padding:0 10px 0 0px;border-width: 0 1px 1px 1px; border-color:#E1E1E1; border-style: solid;"><?=$balance?></td> <!-- 금일 잔액 -->
							</tr>
							<? } ?>
							<tr bgcolor="#f6f6f6">
								...
								<td align="right" style="padding:0 10px 0 0px;border-width: 0 1px 1px 1px; border-color:#E1E1E1; border-style: solid;"><font color="#000099"><?if($auth_row[group]>$auth_level){echo "조회 권한 없음";}else if($total_ba==0){echo "-";}else{echo number_format($total_ba);}?></font></td>
							</tr>
							</table>
							</td>
							...
							<tr>
								...
								<table border="0" width="100%" cellspacing="0" cellpadding="0">
								<tr bgcolor="#f2f2f9">
									<td colspan="3" style="padding:0 0 0 10px;border-width: 1px 0 1px 1px; border-color:#E1E1E1; border-style: solid;" height="28">
									<b><font color="#ee0066">▶</font> <font color="#003399">금 일 수 지 현 황</font></b> (<?=$e_date?> 현재)</td>
								</tr>
								<tr bgcolor="#f5f5f5">
									<td width="264" align="center" style="padding:0 0 0 10px;border-width: 0 0 1px 1px; border-color:#E1E1E1; border-style: solid;" colspan="3" height="28"> 수 입 내 역 </td>
								</tr>
								<tr bgcolor="#f8f8f3">
								...
								<?
									$da_qry="SELECT cont,acc,inc FROM cms_capital_cash_book WHERE (com_div>0 AND class2<>8) AND (class1='1' or class1='3') AND deal_date='$e_date' order by seq_num";
									$da_rlt=mysql_query($da_qry, $connect);

									for($i=0;$i<$num;$i++){
										$da_rows=mysql_fetch_array($da_rlt);
										if($da_rows[inc]==0){ $income="";}else{$income=number_format($da_rows[inc]);}
										...
										if(!$da_rows[cont]){
											 ...
											 $da_cont=$da_rows[cont];
										}
								?>
								<tr>

									<td style="padding:0 0 0 10px;border-width: 0 0 1px 1px; border-color:#E1E1E1; border-style: solid;" height="28"><?=rg_cut_string($da_acc,9,"..")?></td>
									<td style="padding:0 0 0 10px;border-width: 0 0 1px 1px; border-color:#E1E1E1; border-style: solid;"><?=rg_cut_string($da_cont,13,"..")?></td>
								...
								<?
									$da_qry="SELECT class1,cont,acc,inc,exp FROM cms_capital_cash_book WHERE (com_div>0) AND (class1='2' or class1='3') AND deal_date='$e_date' order by seq_num";
									$da_rlt=mysql_query($da_qry, $connect);

									for($i=0;$i<$num;$i++){
										$da_rows=mysql_fetch_array($da_rlt);
										if($da_rows[exp]==0){ $exp="";}else{$exp=number_format($da_rows[exp]);}
