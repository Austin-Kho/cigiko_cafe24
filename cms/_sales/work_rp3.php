					...
					<div style=" height:18px; background-color:#F8F8F8" class="d3_sub">
						<b><font size="2" color="#cc0099">◈</font><font size="2" color="#6666cc"> 시행사 업무보고</font></b>
					...
					<!-- ============================= subject table end ============================= -->
					<?
						$sa_1_3_rlt = mysql_query("select sa_1_3 from cms_mem_auth where user_id='$_SESSION[p_id]' ", $connect);
						$sa_1_3_row = mysql_fetch_array($sa_1_3_rlt);

						if(!$sa_1_3_row[sa_1_3]||$sa_1_3_row[sa_1_3]==0){
					?>
					...
					<tr>
						...
						<tr>
								...
								<div style="height:435px;">
									<?
										...
										if(!$ss_di||$ss_di==1){// 초기 화면이나 리스트 불러올 때
											include "work_rp3_1.php";
										}else if($ss_di==2){ // 신규 수정등록 화면일 때
											include "work_rp3_2.php";
										}
									?>
								</div>
							</td>
							<td style="width:252px; height:480px; border-width:1px 0 1px 1px; border-style:solid; border-color:#cccccc; background-color:#f8f8f8; padding:5px 5px 5px 5px;" valign="top"><!-- B -->
								<div style="height:50px; margin-bottom:6px; background-color:#e3eae7;"></div>
							...
							<div style="height: 25px; padding:5px 5px 0 5px; margin:0 5px 0 5px;" class="bottom">
								<a href="<?$_SERVER['PHP_SELF']?>?m_di=1&amp;s_di=3" style="color:#000000;"><b><?=$s_di_3?></b></a>
							</div>
							</td>
						</tr>
						</table>
